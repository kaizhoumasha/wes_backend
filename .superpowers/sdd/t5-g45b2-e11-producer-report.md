...
- Celery scanner 逐 demand commit；只接受空 target 或唯一 `WMS_FULFILLMENT`。commit 后 enqueue 失败不会回滚
  durable Outbox。每个已评价 demand（包括无候选且状态不变者）均在本轮加入 exclusion，坏 demand 会继续
  尝试下一条；下一轮仍可重试。
...
- RED：首条无候选的 due demand 未被本轮排除时，scanner 只调用两次便停止，健康第二条遭饥饿（`1 failed`）。
...
- `tests/workline_runtime/test_smt_inbound_handoff_full_box_contract.py` +
  `tests/workline_runtime/test_smt_recovery_task_transaction.py`：`8 passed`。
...
- 修改前对 evaluate、scanner、reserve/prepare/context 执行 impact；前三者中 reserve/prepare/context 因落后 index
  返回 UNKNOWN，按获授权高风险边界实施。提交前以指定 CLI 对 staged diff 复检：5 个符号、0 个受影响流程、LOW。
...
- 另有不在本任务边界的既有 fixture 失败：
  `tests/workline_runtime/test_wms_full_box_exchange_domain_projection.py::test_e11_non_success_opens_reconciliation_before_reducer`
  的 `_ReconciliationProjector` 不接受源码既有传入的 `frozen_ack` 参数，未扩大修复。
