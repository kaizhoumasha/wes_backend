...
def test_only_static_composition_root_imports_plugin_without_mutable_registry() -> None:
    ...
    assert "class RoughSorterPluginFactFactory" in application_sources["factory.py"]
    assert "def build_device_fact" in application_sources["device_facts.py"]
    assert "def build_wms_fact" in application_sources["wms_facts.py"]
    assert "def build_transport_fact" in application_sources["transport_recovery_facts.py"]
    assert "def build_recovery_fact" in application_sources["transport_recovery_facts.py"]
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("operation", "operation_id", "position", "request_data"),
    [
        (
...
async def test_factory_builds_recovery_wms_continuation_from_verified_causal_evidence(
    operation: str,
    operation_id: str,
    ...
    await applier.apply(
        object(),
        recovery,
        factory._executions.execution,  # type: ignore[attr-defined]
        first,
        RecoveryDecidedHandler()(first),
    )

    assert confirmations.requests[0]["request_payload"]["data"] == request_data  # type: ignore[index]
