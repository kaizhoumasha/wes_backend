...
from manual_picking.definition import OUTLET
...
class ManualPickingDrainFlow:
    ...
    async def decide_in_session(self, db: Any, line: Any, now: Any) -> tuple[int, Any]:
        ...
        if current is not None:
            if current.intent.workline_code != line.line_code or current.intent.plugin_key != line.plugin_key:
                raise ValueError("drain frozen WorkLine owner mismatch")
            if any(
                candidate.source_location_code != line.position_bindings[OUTLET.slot_key]["location_id"]
                for candidate in current.intent.return_candidates
            ):
                raise ValueError("drain frozen RETURN_BUFFER binding mismatch")
        ...
        if current is None:
            ...
            if prepared.prepared:
                return 1, None
        location = line.position_bindings[OUTLET.slot_key]["location_id"]
        intent = wms_operations.workline_return_buffer_drain_rack_decide(
            ...
            previous_operation_id=current.intent.operation_id if current is not None else None,
        )
    ...
    async def return_in_session(self, db: Any, line: Any, decision: Any, location: str, now: Any) -> bool:
        ...
        latest = await self._history.latest_return(
            db, workline_id=line.id, rack_id=rack.rack_id, rack_face=rack.rack_face
        )
