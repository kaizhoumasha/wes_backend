    ...
    try:
        async with sessions.begin() as db:
            ...
            evidence = InboundEvidence(
                ...
                payload_digest="b" * 64,
                normalized_payload={"code": "DECIDED", "data": {"result": "NO_BATCH", "retry_after_ms": 1000}},
                received_at=now,
                workline_id=7,
                operation="outbound.bin.inbound_batch@v1",
...
async def test_no_batch_retry_and_face_done_are_derived_from_matched_wms_results() -> None:
    ...
    try:
        async with sessions.begin() as db:
            ...
            assert await repo.inbound_retry_due(db, 7, "PICK-1", "R1", "90", now)
            ...
            inbound_confirmation = WmsConfirmation(
                ...
                request_payload={
                    ...
                    "operation_id": "019f0000-0000-7000-8000-000000000003",
                    "timestamp": 1_788_975_600_000,
                    "data": {"task_id": "PICK-1", "rack_id": "R1", "rack_face": "90", "max_bin_count": 4},
                },
                deadline_at=now,
                status="COMPLETED",
                completed_at=now,
            ...
            await db.flush()
            assert not await repo.inbound_retry_due(db, 7, "PICK-1", "R1", "90", now)
            assert await repo.inbound_retry_due(db, 7, "PICK-1", "R1", "270", now)
            assert await repo.inbound_retry_due(db, 8, "PICK-1", "R1", "90", now)
    finally:
        await engine.dispose()


@pytest.mark.asyncio
async def test_inbound_no_batch_is_not_re_requested_before_retry_deadline() -> None:
    module = import_module("manual_picking.application.batch_repository")
    now = datetime(2026, 9, 13, 12)
    history = type("History", (), {})()
    history.latest_inbound = AsyncMock(return_value=(sdk.BinInboundBatchOutcome(sdk.BinBatchNoBatch(1000)), now))
    repo = module.BatchRepository(history)

    assert not await repo.inbound_retry_due(object(), 7, "PICK-1", "R1", "90", now + timedelta(milliseconds=999))
    assert await repo.inbound_retry_due(object(), 7, "PICK-1", "R1", "90", now + timedelta(milliseconds=1000))
    history.latest_inbound.return_value = (sdk.BinInboundBatchOutcome(sdk.BinInboundBatchRackFaceDone()), now)
    assert not await repo.inbound_retry_due(object(), 7, "PICK-1", "R1", "90", now + timedelta(minutes=1))
