...
from sqlalchemy import BigInteger, UniqueConstraint, delete, event, insert, update
...
def test_passage_identity_and_fifo_indexes() -> None:
    ...
    assert {"workline_id", "task_id", "scan1_evidence_id", "scan3_evidence_id"} <= set(table.columns.keys())
    assert "scan4_evidence_id" not in table.columns
    assert "return_state" not in table.columns
    ...
    assert "ix_manual_picking_passages_return_fifo" not in indexes
...
@pytest.mark.asyncio
async def test_archive_and_closed_passage_cleanup_preserve_return_workload() -> None:
    ...
    passage_table = ManualPickingPassage.__table__
    return_table = BinLineReturn.__table__
    try:
        async with engine.begin() as db:
            await db.run_sync(passage_table.create)
            await db.run_sync(return_table.create)
            await db.execute(
                insert(passage_table).values(
                    workline_id=7,
                    task_id="PICK-001",
                    bin_code="A000000001",
                    scan1_evidence_id=1,
                    scan1_received_at=datetime(2026, 9, 15, 7),
                )
            )
            await db.execute(insert(return_table).values(workline_id=7, bin_code="A000000001", scan3_evidence_id=2))

        sessions = async_sessionmaker(engine, expire_on_commit=False)
        async with sessions.begin() as db:
            repository = PassageRepository()
            assert await repository.archive_open_work(db, workline_id=7, archived_at=datetime(2026, 9, 15, 8)) == 1
            await db.execute(delete(passage_table).where(passage_table.c.disposition == "CLOSED"))
            assert await repository.get_unfinished_workload_summary(db, 7) == {
                "count": 1,
                "sample": "bin-line return",
            }
