"""Validate Phase5 business destructive cleanup ledger and strict absence state."""
...
TARGET_CAPABILITY_NAMESPACES = frozenset({"runtime", "phase4", "external", "workline-config"})
...
LEDGER_AUDIT_DOCS = frozenset(
    {
        ...
        LEDGER_PATH,
        Path("docs/architecture/phase5-business-destructive-cleanup-ledger.md"),
        Path("docs/superpowers/plans/2026-07-07-phase5-business-legacy-destructive-cleanup.md"),
        Path("scripts/generate_legacy_matrix.py"),
        Path("tests/architecture/test_workline_domain_mirror.py"),
        Path("tests/architecture/test_phase5_business_legacy_absence_guardrail.py"),
    }
)

FORBIDDEN_PHASE4_CONTRACT_IMPORT_PREFIXES = (
    "sqlalchemy",
...
def _phase4_matrix_rows(repo_root: Path) -> list[dict[str, str]]:
    _, rows = _read_csv(repo_root / MATRIX_PATH)
    return [row for row in rows if row["phase4_carrier"] == "True"]
...
def phase4_contract_layer_violations(repo_root: Path) -> tuple[str, ...]:
    contracts_root = repo_root / PHASE4_CONTRACTS_ROOT
    ...
    for path in sorted(contracts_root.rglob("*.py")):
        for module in _phase4_contract_imports(repo_root, path):
            ...
            if any(
                ...
                for prefix in FORBIDDEN_PHASE4_CONTRACT_IMPORT_PREFIXES
            ):
...
def _row_state_failures(row: dict[str, str], tracked_files: set[str], repo_root: Path) -> list[str]:
    ...
    if row["phase4_carrier"] != "True":
        failures.append(f"{entry_id}:phase4_carrier must be True")
...
def validate_ledger(repo_root: Path, *, mode: str) -> GateResult:
    ...
    if not (repo_root / LEDGER_PATH).exists():
        return GateResult(False, "PHASE5_BUSINESS_DESTRUCTIVE_LEDGER_MISSING", (str(LEDGER_PATH),))

    matrix_rows = _phase4_matrix_rows(repo_root)
    ...
    failures.extend(phase4_contract_layer_violations(repo_root))

    if failures:
        return GateResult(False, "PHASE5_BUSINESS_DESTRUCTIVE_CLEANUP_OPEN", tuple(failures))
    return GateResult(True, f"PHASE5_BUSINESS_DESTRUCTIVE_CLEANUP_{mode.upper()}_READY")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not result.valid:
        print(f"Phase5 business destructive cleanup gate failed: {result.reason}")
        for detail in result.details[:80]:
            print(f"- {detail}")
        if len(result.details) > 80:
            print(f"- ... {len(result.details) - 80} more")
        return 1
    print(f"Phase5 business destructive cleanup gate passed: mode={args.mode}")
    return 0
