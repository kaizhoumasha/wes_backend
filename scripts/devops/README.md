...
gitlab_rails['ldap_servers'] = {
  'main' => {
    'label' => 'Windows AD', # 登录页面显示的名称
    'host' =>  '192.168.0.202', # LDAP 服务器地址
    'port' => 389,
    'uid' => 'sAMAccountName', # Windows AD 必须使用 'sAMAccountName'
    'bind_dn' => 'CN=周凯,OU=平台组,OU=产品部,OU=研发处,OU=灿态信息科技,DC=ct,DC=com', # 用于查询的管理员账号
    'password' => 'xxxxxxx',
    'encryption' => 'plain', # 可选 'start_tls', 'simple_tls', 'plain'
    'base' => 'DC=ct,DC=com', # 搜索根节点
...
2. **配置安全域**:
   ...
   - 填写配置信息：
     - **Server:** `ldap://192.168.0.202` (如果使用 SSL 则是 `ldaps://...`)
     - **root DN:** `DC=ct,DC=com`
     - **User search base:** (留空通常即可，或指定 ou)
     - **User search filter:** `sAMAccountName={0}`
     - **Manager DN:** `CT\zhoukai`
