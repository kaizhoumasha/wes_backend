...
- [WES - WMS 对接接口需求](docs/integration/wes-wms-interface-requirements.md)：WES/WMS 公共接口外发真源；当前仅公共协议和 Transport 为 `Approved`，WES 实现仍为 `ALIGNMENT_REQUIRED`，业务附录保持 `ReviewRequired`。
...
- [WMS / WES 自动入库与上架交互要求](docs/contracts/wms-inbound-putaway-integration-requirements.md)：粗分逐盘入库、满箱交换和自动上架的业务合同评审真源；当前为 `ReviewRequired`，不代表已实施。
- [WES 架构收敛十二阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：Phase 6 Transport 与 Phase 7 DeviceCommand/ECS 核心生产基线已完成；Phase 8/9 在真实设备合同获批后交付独立业务插件。
