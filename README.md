...
- [WMS 业务 ACL 收敛计划](docs/superpowers/plans/2026-08-05-wes-wms-thin-access-convergence.md)：Phase 3 无状态、消费者驱动的暗构建入口；隔离局域网 inbound `NONE` 已冻结，Task 1 三项跨能力外部门禁未关闭，关闭后每个 operation 使用独立小 PR 实施。
- [WORKLINE 业务插件二次开发指南](docs/plugin_development_guide.md)：最小插件 SPI、封闭 Decision 与独立插件包交付约定。
- [WMS 业务 ACL 交互合同](docs/contracts/wms-northbound-interaction-contract.md)：以真实消费者为准收敛查询、业务确认和 inbound 命令；搬运与 RCS 状态归 Phase 4。
