...
- [WES 出库操作顶层设计](docs/superpowers/specs/2026-08-06-wes-outbound-operation-top-level-design.md)：待 WMS 书面复审的 `PickingTask` 候选方案，不是已批准业务或 wire 真源。
...
- [WORKLINE 执行插件二次开发指南](docs/plugin_development_guide.md)：最小插件 SPI、WMS 业务结果到封闭执行 Decision 的映射与独立插件包交付约定。
