...
- [WES - WMS 对接接口需求](docs/integration/wes-wms-interface-requirements.md)：WES/WMS 公共接口外发真源；粗分插件已从代码库移除，运行环境未变更，其余业务附录按各自状态评审。
...
- [WES 架构收敛十四阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：阶段性实施记录；当前状态以代码和仍在项目内的合同为准。
- [WORKLINE 业务插件二次开发指南](docs/plugin_development_guide.md)：最小插件 SPI、封闭 Decision 与独立插件包交付约定。
- [人工拣料插件安装与装配](workline_plugins/manual-picking/README.md)：`manual-picking` 的设备/工作位声明、安装、`prepare`、计划进场和 Transport 结果边界。
...
- [AGV/CTU 通用搬运能力合同](docs/contracts/transport-fulfillment-contract.md)：定义四类搬运请求、同步接纳 ACK、成员位置事实、异步最终结果、幂等和对账边界；业务 producer 由当前启用的插件显式装配。
