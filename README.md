...
- [WES 架构收敛十二阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：Phase 5 先退役旧插件执行闭包，Phase 6/7 分别收敛 Transport 与 Device/ECS，Phase 8/9 重写独立插件。
...
- [AGV/CTU 通用搬运能力合同](docs/contracts/transport-fulfillment-contract.md)：定义已批准的 Phase 4 四类搬运请求、同步接纳 ACK、成员位置事实、异步最终结果、幂等和对账边界；当前实现保持暗构建，不接生产流量。
