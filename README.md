...
- [WES 架构收敛十四阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：Phase 6 Transport 与 Phase 7 DeviceCommand/ECS 核心生产基线已完成；Phase 8 backend RC 已 `CLOSED`；Phase 9 已合入 `develop@c5a93872`；Phase 10 已由 #187 合入 `develop@97e6887a`，但该 merge commit 未证明重新部署；Phase 11 的 #188/#189 已完成并到达 `develop@d458383a`，状态为 `MERGED — NOT DEPLOYED`，旧 `dd35f04b258f` 数据库不能原地升级。Transport 0.3 与 510056 backend 修复已通过 QUALITY 与 selected HEAVY，当前为 `IMPLEMENTED — IMMUTABLE IMAGE E2E PENDING`，frontend canonical sync 等待 backend Land；WMS 外部发布、当前 HEAD 的 Phase 8 不可变 E2E、供应商一致性、设备物理和业务验收均未完成。Phase 8 分层证据见[粗分机后端开发验收与现场边界状态](docs/integration/rough-sorter-joint-acceptance.md)。
