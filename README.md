...
- [Phase 9 后持续业务交付阶段重排设计](docs/superpowers/specs/2026-08-21-phase9-continuous-business-delivery-resequence-design.md)：已批准的 Phase 9 人工纵向切片、Phase 10/11 清理与单次基线、Phase 12 自动插件和 Phase 13 验收顺序。
...
- [WMS / WES 自动入库与上架交互要求](docs/contracts/wms-inbound-putaway-integration-requirements.md)：Phase 12 满箱交换和自动上架的业务合同评审真源；当前为 `ReviewRequired`，不代表已实施。
- [WES 架构收敛十三阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：Phase 6 Transport 与 Phase 7 DeviceCommand/ECS 核心生产基线已完成；Phase 8 backend RC 已 `CLOSED`；Phase 9 严格合同和详细计划已批准，生产代码尚未开始；Phase 10—13 尚未开始。供应商一致性、现场联调和业务验收仍为 `NOT RUN`，分层证据见[粗分机后端开发验收与现场边界状态](docs/integration/rough-sorter-joint-acceptance.md)。
