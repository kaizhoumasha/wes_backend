...
- [WMS Adapter 新能力重建计划](docs/superpowers/plans/2026-08-05-wes-wms-thin-access-convergence.md)：Phase 3 暗构建入口；E02 已批准使用 `POST`，代码实施仍被完整字段、其余 19 项与 E08–E14 异步关联合同阻断。
- [WORKLINE 业务插件二次开发指南](docs/plugin_development_guide.md)：最小插件 SPI、封闭 Decision 与独立插件包交付约定。
- [WMS 北向 35 项 Operation 合同](docs/contracts/wms-northbound-interaction-contract.md) 与 [全工厂业务蓝图](docs/business/wms_full_factory_operation_blueprint.md)：16 项采用初稿 wire 基线；完整 DTO 字段矩阵、其余 19 项及 E08–E14 异步关联合同待批准。
