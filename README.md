...
- [WES 架构收敛十四阶段总控](docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md)：Phase 6 Transport 与 Phase 7 DeviceCommand/ECS 核心生产基线已完成；Phase 8 backend RC 已 `CLOSED`；Phase 9 已合入 `develop@c5a93872`；Phase 10 Tasks 0–7 已完成，`834fe59e` 候选已部署联调并通过 target-only cutover，发布分支已推送但尚未合入 `develop`。Phase 11 仍须独立准入评审；供应商一致性、设备物理和业务验收未完成。Phase 8 分层证据见[粗分机后端开发验收与现场边界状态](docs/integration/rough-sorter-joint-acceptance.md)。
