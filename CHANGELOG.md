...
- KT16 后续退箱不再被已交接任务缺少 SOURCE_PICKED 或历史异常响应阻塞，退箱分配同时校验当前活跃任务的储位冲突。
- SCAN3 可作为异常恢复入口，新的实际扫码建立当前回程，旧轮次和迟到回调不覆盖新轮次；无旧位置记录的料箱也可完成 SCAN4 后进入退箱队列。
- SCAN1 不再前置等待目标货架；CTU03 仅在当前有效搬箱依赖解除后离场，已取消成员的最终空清单正常闭合。

### Changed

- 离场决定和 WMS 确定结果持久化后立即唤醒派发与重新求值，保留原 10 秒扫描作为恢复兜底，不新增业务状态或缩短轮询周期。

### Added

- KT16 现场 BAD CASE 与完整六箱 GOOD CASE 验收依据，包含跨系统时间线、原始证据及明确的现场验证边界。

## [0.49.1.1] - 2026-09-25

### Fixed

- 无业务插件的 ECS_TEST 工作线启动成功后正常返回当前状态，不再因空插件字段错误返回 422。
...
- 手工出库联调台将 WES `sorting-3` 与 WMS prepare 的 `data.workline_code` 分开，后者由管理员明确填写并默认使用现场代码 `KT16`。
- WMS 明确未接收后，同参数重发保留原 `operation_id` 和正文；参数改正则保留旧请求审计并使用新 UUIDv7 身份，已保存有效 WMS 响应的请求仍禁止重发。

### Verification

- WmsConfirmation 生命周期、人工出库联调状态机、WMS 派发、API、迁移与 PostgreSQL 聚焦测试通过。
...
- 工作线可独立维护货架工作位及其关联设备，支持退料、转运货架位；工作位容量表示可容纳的货架数量。

### Changed

- 基础配置与业务插件装配分别保存，更换或解除插件保留工作位和设备归属；移除插件正在引用的设备前需先解除角色绑定。
- 两类配置共用工作线版本、停用和运行负载校验。原配置接口仅接收插件配置，前端需同步新契约；配套前端必须在新后端之后部署。
...
- 自动联调支持指定已启用工作线，由 WMS 为回架料箱分配目标槽位，并提供成功回架后的实际储位，供后续独立轮次接续使用。
- WMS Mock 支持回架分配、已释放槽位选择和入线至回架交接位的显式到位模拟，可验证料箱换位后的连续联调。

### Fixed

- 扫码 Evidence 阻塞完成重处理后，自动联调可沿原轮次继续执行。
- 回架任务创建前工作线校验失败时，修正配置后可恢复原步骤，保留原执行身份及未知状态保护。

### Verification

- API 与自动联调状态机聚焦回归：101 passed；最终 staged manifest 选择的 HEAVY：161 passed。
- 本机前后端及 ECS/WMS Mock 验证连续两轮与关闭页面后的当前轮收尾，共 3 个独立轮次、27 个步骤、21 个运输任务成功；后续轮次使用已确认的新槽位。
- 无数据库迁移；升级前需完成既有自动联调任务。前端正式契约冻结、合并、部署及现场验收不在本次后端交付范围。
...
- Jenkins 工作区改为在流水线 shell 内初始化并精确 fetch/checkout source 与 target refs，阻断 GitLab 插件通过 `RevisionParameterAction` 动态注入 `PreBuildMerge`；后续构建和验证始终从 webhook 已核验的 source Commit 执行。

### Verification

- Jenkins 实跑确认旧 `checkout` 步骤仍会被插件注入 merge 后新增回归合同；新路径不再调用 Jenkins GitSCM `checkout`，并保留 source SHA、TLS target SHA 与对象存在性 fail-closed 检查。
...
- 设备接入诊断流只在 Redis 订阅真正就绪后建立连接，并为发布、订阅启动与清理设置明确边界，避免客户端或 worker 长时间挂起。
- 现场联调命令继续复用既有 `DeviceCommand`、ECS 状态查询与准入链路；活动真实命令存在时，Mock 故障状态不再改写设备事实。
