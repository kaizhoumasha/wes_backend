...
**规则**：每次更新或增加功能后，**必须**同步更新 `docs/file_index.md` 文档。
...
**同步工具**：使用 Serena MCP 分析项目结构
```bash
# 1. 激活项目（如果未激活）
serena activate wes_backend

# 2. 分析项目结构
serena list_dir . --recursive --skip-ignored

# 3. 对比并更新 docs/file_index.md
# 4. 更新同步日志（版本号、变更内容）
```
...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| YYYY-MM-DD | v2.x | 变更描述 |
```

**验证方法**：
```bash
# 检查文档是否与代码库同步
# 1. 对比 docs/file_index.md 中的文件列表
# 2. 使用 Serena list_dir 验证文件存在性
# 3. 确保没有遗漏的新文件
```
