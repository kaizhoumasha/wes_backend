...
from src.core.schema_loader import apply_schema_loads, get_with_schema, get_all_with_schema
from src.app.admin.models import User, Role, RoleRead
...
async def example_get_multiple(db: AsyncSession):
    """示例3: 使用便捷函数查询多个对象"""
    roles = await get_all_with_schema(
        db,
        Role,
        RoleRead,
        Role.is_active == True,
