...
def register_custom_route(
    ...
    async def create_application(  # pyright: ignore[reportUnusedFunction]
    ...
    async def revoke_application(  # pyright: ignore[reportUnusedFunction]
        ...
        if not app:
            return cast(
                "ResponseSchemaModel[None]",
                response_builder_any.fail(code=ResourceErrorCode.NOT_FOUND, message=f"应用不存在: {id}"),
            )

        success = await api_app_service.revoke_app(db, app.app_id, cache)
        if success:
            return cast("ResponseSchemaModel[None]", response_builder_any.success(message="应用已撤销"))
        return cast(
            "ResponseSchemaModel[None]",
            response_builder_any.fail(code=BusinessErrorCode.OPERATION_FAILED, message="撤销失败"),
        )
    ...
    async def reset_validity_period(  # pyright: ignore[reportUnusedFunction]
        db: AsyncSessionDep,
        cache: CacheDep,
        id: int,
    ...
    async def try_invoke_application(  # pyright: ignore[reportUnusedFunction]
        ...
        请求格式：{"data": {...}}
        """
        result_data: dict[str, Any] = {"app_ctx": app_ctx, "data": request_data.data}
        return cast(
            "ResponseSchemaModel[dict[str, Any]]",
            response_builder_any.success(data=result_data, message="API 调用成功"),
        )
    ...
    async def assign_permissions(  # pyright: ignore[reportUnusedFunction]
        id: Annotated[int, Path(...)],
    ...
    async def reset_secret(  # pyright: ignore[reportUnusedFunction]
        id: Annotated[int, Path(...)],
        db: AsyncSessionDep,
        cache: CacheDep,
