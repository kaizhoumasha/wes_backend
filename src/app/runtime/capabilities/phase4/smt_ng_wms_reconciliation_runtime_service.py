"""Phase4 SMT/NG/WMS reconciliation runtime capability.
...
from src.app.runtime.capabilities.phase4.sorter_inbound_runtime_service import Phase4RuntimeCapabilityPlan
...
class SmtNgWmsReconciliationRuntimeService:
    """Phase4 SMT/NG/WMS 对账 runtime capability builder。"""

    def build_reconciliation_plan(self, payload: Mapping[str, Any]) -> Phase4RuntimeCapabilityPlan:
        """构建 callback 对账运行计划。"""
        ...
        intents = [
            RuntimeIntent.resource_fact(
                ...
                idempotency_key=f"phase4:{source_event_id}:reconciliation-evidence",
            )
        ]
        ...
        if reconciliation_required:
            intents.append(
                RuntimeIntent.block(
                    ...
                    message="Phase4 reconciliation requires manual or worker resolution",
                    payload={
                        "scope_type": "OBJECT",
    ...
    def build_runtime_hold_release_plan(self, payload: Mapping[str, Any]) -> Phase4RuntimeCapabilityPlan:
