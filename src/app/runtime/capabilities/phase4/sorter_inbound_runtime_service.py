...
class Phase4RuntimeCapabilityPlan(BaseModel):
    """Phase4 runtime capability 输出计划。"""
...
class Phase4SorterInboundRuntimeService:
    """Phase4 sorter inbound runtime capability builder。"""

    def build_rough_sorter_inbound_plan(self, payload: Mapping[str, Any]) -> Phase4RuntimeCapabilityPlan:
        """构建粗分机入库运行计划。"""
        ...
        location_payload = {
            ...
            "source_event_id": _text(payload.get("source_event_id")),
            "source_version": _text(payload.get("source_version")),
            "idempotency_key": f"phase4:{request_id}:location-fact",
        }
        pkg_binding_payload = {
            "package_id": pkg_code,
            "pallet_id": pallet_id,
            ...
            "provider_code": provider_code,
            "correlation_id": correlation_id,
            "idempotency_key": f"phase4:{request_id}:pkg-binding",
        }
        inventory_payload = {
            ...
            "provider_code": provider_code,
            "correlation_id": correlation_id,
            "idempotency_key": f"phase4:{request_id}:inventory-confirm",
        }

        return Phase4RuntimeCapabilityPlan(
            provider_code=provider_code,
            intents=[
    ...
    def build_sorter_inbound_plan(self, payload: Mapping[str, Any]) -> Phase4RuntimeCapabilityPlan:
        """构建分拣机入库运行计划。"""
            ...
            return Phase4RuntimeCapabilityPlan(
                ...
                intents=[
                    RuntimeIntent.block(
                        ...
                        message="Phase4 sorter inbound join gate is not satisfied",
                        payload={
                            "request_id": request_id,
                            "scope_type": "OBJECT",
    ...
    def build_full_box_exchange_plan(self, payload: Mapping[str, Any]) -> Phase4RuntimeCapabilityPlan:
        """构建满箱交换运行计划。"""
        ...
        operation_key = f"phase4:{request_id}:full-box-exchange"
...
phase4_sorter_inbound_runtime_service = Phase4SorterInboundRuntimeService()
