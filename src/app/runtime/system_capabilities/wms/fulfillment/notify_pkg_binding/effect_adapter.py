"""`notify_pkg_binding` DispatchEnvelope 到既有 T8 双账本的薄适配。"""
...
class NotifyPackageBindingEffectAdapter:
    """只冻结出站 binding 并构造 SystemOutbox；持久化由 orchestration Service 负责。"""

    def __init__(self, *, gateway: NotifyPackageBindingDispatchGateway | None = None) -> None:
        self._gateway = gateway or NotifyPackageBindingDispatchGateway()
    ...
    def build_outbox(
        ...
        if frozen_binding is None:
            raise ValueError("notify_pkg_binding requires frozen EXTERNAL_HTTP binding")
        return SystemOutbox(
...
notify_package_binding_effect_adapter = NotifyPackageBindingEffectAdapter()
