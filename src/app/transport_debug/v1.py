...
from src.app.transport_debug.v1_tasks import router as debug_tasks_router

router = APIRouter(tags=["Transport 调试"])
router.include_router(debug_tasks_router)
...
def _debug_run_service(request: Request) -> Any:
    ...
    service = getattr(request.app.state, "transport_debug_run_service", None)
    if service is None:
