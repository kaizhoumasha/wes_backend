...
def _reset_service(request: Request) -> Any:
    _transport_runtime(request)
    service = getattr(request.app.state, "transport_debug_reset_service", None)
    if service is None:
        raise ServiceUnavailableException("Transport debug reset service 不可用")
    return service
...
async def preview_debug_transport_task_reset(
    ...
    preview = await _reset_service(request).preview_debug_task_reset(transport_task_id)
...
async def reset_debug_transport_task(
    ...
    try:
        if confirmation is None:
            result = await _reset_service(request).reset_debug_task(transport_task_id)
        else:
            result = await _reset_service(request).reset_debug_task(
                transport_task_id,
                TransportDebugStepConfirmation(
