...
class PickingTaskCancelPersistenceResult:
    code: str
    timestamp_ms: int
    reason_code: str | None = None


class PickingTaskCancelRecorder(Protocol):
    async def record(
        self,
...
class PickingTaskCancelHandler:
    def __init__(self, recorder: PickingTaskCancelRecorder) -> None:
    ...
    async def handle(self, raw_envelope: dict[str, Any], *, observation: WmsCallObservation | None = None) -> EventAck:
        ...
        if operation != PICKING_TASK_CANCEL_OPERATION:
            return event_ack(
                422, operation_id, "REJECTED", response_timestamp, {"reason_code": "UNSUPPORTED_OPERATION"}
            )
        envelope = parse_picking_task_cancel_receipt(raw_envelope, observation=observation)
        try:
            persisted = await self._recorder.record(envelope, received_at=timezone.now_for_db())
        except Exception:
            logger.exception("PickingTask 取消事件持久化失败: operation_id=%s", operation_id)
        ...
        if response is not None:
            return response
        logger.error("PickingTask 取消事件持久化返回未知结果: %r", persisted)
        return event_ack(503, operation_id, "UNAVAILABLE", response_timestamp, {})
