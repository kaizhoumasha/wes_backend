...
def parse_bin_inbound_batch_request(value: object) -> BinInboundBatchRequest:
    return BinInboundBatchRequest.model_validate(value)


def parse_bin_inbound_batch_response(
    status_code: int, value: object, *, request: BinInboundBatchRequest | None = None
) -> BinInboundBatchResponse:
    code = value.get("code") if isinstance(value, dict) else None
    adapter = _RESPONSE_ADAPTERS.get((status_code, code)) if isinstance(code, str) else None
    if adapter is None:
        raise ValueError("HTTP status 与 inbound_batch response code 不匹配")
    ...
    if request is not None:
        ...
        if isinstance(response, BinInboundBatchDecidedResponse) and isinstance(response.data, BinInboundBatchReady):
            if len(response.data.bins) > request.data.max_bin_count:
                raise ValueError("READY Bin 数量超过请求容量")
            if any(
                ...
                raise ValueError("READY 来源必须匹配请求货架面")
    return response
