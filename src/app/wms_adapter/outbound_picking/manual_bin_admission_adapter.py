...
class ManualBinAdmissionAdapter:
    ...
    async def dispatch(
        ...
        *,
        ...
        try:
            request = parse_manual_bin_admission_request(request_payload, observation=observation)
        ...
        if response.code == "DECIDED":
            # WAIT 是确定响应；人工联调由操作员使用新 identity 重求值。
            # 共享 dispatcher 不重放旧 identity。
            return WmsDispatchResult(
...
__all__ = ["ManualBinAdmissionAdapter"]
