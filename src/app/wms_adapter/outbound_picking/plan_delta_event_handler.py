"""PickingTask 计划增量事件的严格接收与 ACK 映射。"""
...
class PickingTaskPlanDeltaPersistenceResult:
    code: str
    timestamp_ms: int
    reason_code: str | None = None


class PickingTaskPlanDeltaRecorder(Protocol):
    async def record(
        self,
        envelope: PickingTaskPlanDeltaEvent | PickingTaskPlanDeltaInvalidData,
        *,
        received_at: datetime,
    ) -> PickingTaskPlanDeltaPersistenceResult: ...


class PickingTaskPlanDeltaHandler:
    ...
    async def handle(self, raw_envelope: dict[str, Any], *, observation: WmsCallObservation | None = None) -> EventAck:
        ...
        if operation != PICKING_TASK_PLAN_DELTA_OPERATION:
            return event_ack(
        ...
        try:
            ...
            logger.exception("PickingTask 计划增量事件持久化失败: operation_id=%s", operation_id)
        ...
        if response is not None:
            return response
        logger.error("PickingTask 计划增量事件持久化返回未知结果: %r", persisted)
        return event_ack(503, operation_id, "UNAVAILABLE", response_timestamp, {})
