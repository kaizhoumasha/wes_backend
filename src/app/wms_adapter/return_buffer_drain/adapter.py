"""`workline.return_buffer.drain_rack_decide@v1` 可靠派发适配器。"""
...
class ReturnBufferDrainAdapter:
    """校验冻结请求，通过共享 WmsClient 单次发送并解释 drain 响应。"""
    ...
    async def dispatch(
        ...
        *,
        ...
        try:
            request = parse_request(request_payload, observation=observation)
        ...
        access = await receive_json(
            self._client, DECISION_PATH, request.model_dump(mode="json", exclude_unset=True), observation=observation
        )
