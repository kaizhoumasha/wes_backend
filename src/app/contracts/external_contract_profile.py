...
设计理由 (AP1 ADR-0009):
- 共享 contract 层避免 capability implementation import boundary 误报 (tests/support/ 升级到 src/app/wms_integration/models/
  会让 wms_integration 内部域反向被 runtime 引用, 触发 WMS integration and capability implementation import boundaries)
...
class InboundNormalizerProfile(BaseModel):
    ...
    是出站业务调用。I3 不变量 + capability dependency guardrails 禁止 normalizer 进入业务 capability。
    """
    ...
    def _normalizer_injection_boundary(self) -> InboundNormalizerProfile:
        ...
        注入 inbound normalizer (capability dependency and inbound normalizer ownership guardrails)。
        """
