...
class PickingTaskCancelRepository:
    ...
    async def cancel_members(
        ...
        *,
        ...
        requested_picks = {
            (source.plan_revision, source.rack_id, source.rack_face, slot_id)
            for source in data.direct_pick_sources or ()
            for slot_id in source.slot_ids
        }
        requested_racks = {
            (source.plan_revision, source.rack_id, rack_face)
            for source in data.bin_source_racks or ()
            for rack_face in source.rack_face
        }
                ...
                (
                    await db.scalars(
                        ...
                        .where(
                            ...
                            tuple_(direct.plan_revision, direct.rack_id, direct.rack_face, direct.slot_id).in_(
                                list(batch)
                            ),
                        )
                        .with_for_update()
                ...
                (
                    await db.scalars(
                        ...
                        .where(
                            ...
                            tuple_(racks.plan_revision, racks.rack_id, racks.rack_face).in_(list(batch)),
                        )
                        .with_for_update()
                    )
                ).all()
            )
        member_bindings = sorted({(row.source_evidence_id, row.rack_id) for row in (*direct_rows, *rack_rows)})
        bindings = TransportDecisionBinding.__table__.c
        transports = TransportTask.__table__.c
        transport_task_ids: list[str] = []
        for batch in batched(member_bindings, MEMBER_BATCH_SIZE, strict=False):
            transport_task_ids.extend(
                str(value)
                for value in (
                    await db.scalars(
                        ...
                        .where(
                            ...
                            tuple_(bindings.source_evidence_id, bindings.resource_fence_id).in_(list(batch)),
                        )
        ...
        await db.flush()
        return bool(direct_rows or rack_rows), tuple(transport_task_ids)
