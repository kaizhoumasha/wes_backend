...
def _validate_worker_queue_contract(actual_queues: frozenset[str]) -> None:
    """实际消费队列必须与声明一致，fulfillment worker 保持单并发。"""

    if _declared_worker_queues() != actual_queues:
        raise ValueError("declared worker queues do not match the actual consume queues")
    if "wms-fulfillment" in actual_queues and actual_queues != frozenset({"wms-fulfillment"}):
        raise ValueError("wms-fulfillment must be consumed by a dedicated worker")
    if actual_queues == frozenset({"wms-fulfillment"}) and os.getenv("CELERY_WORKER_CONCURRENCY", "").strip() != "1":
...
def on_beat_init(*args: Any, **kwargs: Any) -> None:
    """Beat 只装配 target schedule 和日志，不创建 outbound client。"""
    try:
        setup_logger()
    except Exception as exc:
