...
本文是 33 项 WMS wire operation 的业务视图。精确 wire 合同以
...
- Q01–Q15 由 `WmsCapabilities` 返回同步类型化结果。
- E01–E07/E15/E17–E18 由可靠对象 `WmsConfirmation` 持久化义务，再调用无状态 sender 获取同步结果。
...
上线前必须证明：33 个垂直模块无缺失/重复，Q15 无副作用，10 个同步确认、7 个异步搬运与 1 个取消操作的
