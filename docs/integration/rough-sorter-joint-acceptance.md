...
**BACKEND FUNCTIONAL IMPLEMENTATION PASS；LOCAL MOCK ACCEPTANCE PASS；BACKEND RC SOURCE READY — IMAGE NOT PUBLISHED**（2026-08-20）。

后端功能实现、本机 Mock 验收、最终快照验证和 GitLab PUSH-only 发布边界已经完成；当前只剩 Commit 与 GitLab PUSH 触发实际
后端镜像发布。前端在独立仓库实现和发布，不要求与后端版本或进度一一对应。真实 WMS/RCS/ECS、设备、版本选择、Docker
部署和联合验收由现场人员负责，不参与后端 RC 关闭。Mock 只证明 WES 按批准合同执行，不替代供应商实现、物理事实或业务验收。
...
| 后端 RC 镜像 | NOT PUBLISHED | Jenkins 仅允许 GitLab `PUSH` 发布带 Commit/source-tree 标签的后端镜像；MR 与手工构建只验证、不发布 | 未获 Commit/Push 授权前不得宣称 RC 已关闭 |
