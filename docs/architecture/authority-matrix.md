...
WES 不是所有外部事实的唯一权威。**按事实类型拆分权威来源**——所有业务资格、来源、目标、优先级、路线、
NG/等待/替代、取消、恢复和业务终态均由 WMS 给出封闭结果；WES 只聚合 evidence、维护作业期投影，并对已获授权的
业务结果作执行决策。WES 不复制外部主数据，也不把本地规则、投影或人工操作升级为业务裁决。
...
| 9 | WMS 业务决策、业务命令与普通事件 | WMS | 决策/命令 DTO normalize 后交给具体执行消费者；普通事件按批准合同 dispatch | 保存 WMS 结果和执行 evidence；不得在本地重算、替换或扩大业务结果 | `authority=WMS` |
| 10 | 执行冲突与人工清线 | WES | 冲突证据 + 作业期执行处置 | 暂停、隔离、清线 evidence；涉及来源、目标、路线、库存或业务终态的处置仍须 WMS 决定 | `authority=WES, scope=EXECUTION_LOCAL` |
...
❌ **错误**：插件查询物料、GRN、库存和本地投影后，自行选择来源、目标格、业务路线、优先级或 NG/等待分支。
