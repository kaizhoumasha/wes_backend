...
- `ScenarioRecorder` / `ScenarioReplayRunner`：补齐脱敏录制、deterministic replay、timeline/outbox/projection hash/reconciliation reason 断言合同；本分支新增 `tests/resilience/fixtures/phase3_runtime_replay_fixture.json`、`phase3_simulator_replay_fixture.json` 与显式 resilience replay 测试；生产数据源采集和 IntegrationLab 完整链路仍待接入。
...
- `uv run pytest tests/resilience/test_phase3_scenario_replay.py -q`：2 passed
...
| ENG-020 | 🟡 部分完成 | 本分支新增 `ScenarioRecorder` / `ScenarioReplayRunner` 脱敏录制和 deterministic replay 合同，并补齐 `tests/resilience/fixtures/phase3_runtime_replay_fixture.json`、`phase3_simulator_replay_fixture.json` + `tests/resilience/test_phase3_scenario_replay.py` 显式回放；生产录制源采集与 IntegrationLab 完整链路仍待补齐 |
...
- [ ] Conveyor queue writer 并发、幂等、诊断和严格模式已覆盖 PostgreSQL 语义。🟡 本分支补齐 writer 决策合同；PostgreSQL 行级锁/upsert 与 IntegrityError 重读仍待补齐。
...
- [ ] ScenarioReplayRunner 能 deterministic 复现关键异常并断言 active projection diff、timeline、outbox/effect 幂等和 reconciliation 结果。🟡 本分支补齐纯 replay 合同、resilience fixture 与 simulator fixture 回放；生产录制源采集仍待补齐。
