...
- `uv run pytest --collect-only -q -o addopts='' | tail -5`：1613 tests collected
- `./scripts/git-quality-gate.sh --profile quality`：通过
...
- [ ] Conveyor queue writer 并发、幂等、诊断和严格模式已覆盖 PostgreSQL 语义。🟡 本分支补齐 writer 决策合同、DB-backed 写入、IntegrityError 重读、结果诊断和 lightweight benchmark artifact 诊断口径；benchmark profile metadata gate 已阻止 lightweight artifact 冒充 PostgreSQL 生产基准；PostgreSQL 行级锁/upsert 与高并发真实基准数据仍待补齐。
...
- [ ] ScenarioReplayRunner 能 deterministic 复现关键异常并断言 active projection diff、timeline、outbox/effect 幂等和 reconciliation 结果。🟡 本分支补齐纯 replay 合同、resilience fixture、simulator fixture 回放、TraceQueryResult 生产录制源适配和 active projection diff 输出；IntegrationLab 完整链路仍待补齐。
