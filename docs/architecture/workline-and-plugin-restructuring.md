...
| Runtime evidence readiness gate | ✅ 开发/测试与 evidence profile 门禁已接入 | `scripts/check_runtime_evidence_readiness_gate.py` 验证 SPEC 状态、material-flow mock 语义基线、preview 边界、runtime capability 文件和 `simulator/site/production` evidence artifact；已接入 `./scripts/git-quality-gate.sh --profile quality` | profile 只改变 evidence 要求，不改变 service 行为；`production` profile 继续叠加 Phase3 production closure gate |
