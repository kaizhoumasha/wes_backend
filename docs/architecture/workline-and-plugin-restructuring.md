...
| ENG-016 | 🟡 部分完成 | 本分支补齐 `ConveyorQueueWriter` 写入决策合同，并新增 runtime DB-backed `ConveyorQueueMembershipWriterService` / repository，覆盖幂等重放、placeholder resolve、跨队列 RECONCILING、strict-mode unknown queue、IntegrityError existing 重读和结果诊断；PostgreSQL 高并发基准与 `tests/load/` 性能数据仍待补齐 |
...
- [ ] Conveyor queue writer 并发、幂等、诊断和严格模式已覆盖 PostgreSQL 语义。🟡 本分支补齐 writer 决策合同、DB-backed 写入、IntegrityError 重读和结果诊断；PostgreSQL 行级锁/upsert、高并发基准和真实 `tests/load` 性能数据仍待补齐。
