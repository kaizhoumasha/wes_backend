...
| ENG-009 | 🟡 部分完成 | RuntimeInbox source event 幂等、payload hash 冲突、唯一冲突重读和审计已覆盖；本分支补齐 `IdempotencyOperationSpec` canonical/alias 审计矩阵、`IdempotencyConflict` 409 payload 与 ReconciliationManager `register_conflict_idempotent()` 登记入口；fulfillment / device event / runtime reconciliation CRITICAL 热路径全量接入仍待补齐 |
...
- [ ] idempotency 跨域语义统一，覆盖 callback / fulfillment / device_command / device_event / reconciliation。🟡 PR #73 已覆盖 callback / RuntimeInbox source event 幂等与 payload hash conflict；本分支补齐 canonical/alias 审计矩阵、409 audit payload 与 ReconciliationManager 幂等登记入口；fulfillment / device_event / runtime reconciliation CRITICAL 热路径生产接入仍待补齐。
