...
- `RuntimePhase3ClosureGate`：补齐 Phase 3 总门禁与 `scripts/check_phase3_closure_gate.py`，同时要求生产 P0 E2E artifact 和 production-scale benchmark artifact 存在并通过各自 gate；缺任一 artifact 或传入 lightweight / sandbox 证据都不能关闭 Phase 3。
...
- `uv run pytest tests/runtime/orchestration/test_phase3_closure_evidence_gate.py -q`：3 passed
...
- `uv run pytest --collect-only -q -o addopts='' | tail -5`：1631 tests collected
- `./scripts/git-quality-gate.sh --profile quality`：通过
