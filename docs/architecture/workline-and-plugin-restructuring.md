...
- [ ] plane 接口安全门禁：`biz:workline:view-plane-scene` / `biz:workline:view-plane-snapshot` + 行级过滤 + 脱敏 + 审计。🟡 PR #73 已落地 `PlaneSceneView` / `PlaneSnapshot` 读模型和 route；本分支补齐 `PlaneReadSecurityPolicy`，集中声明专用权限、`WORKLINE_LOCAL` scope、脱敏字段 deny-list 与审计 action，并将 route 权限依赖改为引用该 policy；本分支继续接入 scene/snapshot read audit 落库；真实行级过滤执行仍待补齐。
...
- `PlaneReadSecurityPolicy`：补齐 plane scene/snapshot 专用权限、`WORKLINE_LOCAL` scope、WorkLine 配置字段脱敏 deny-list 与审计 action 的集中安全合同；route 权限依赖从硬编码字符串收敛为引用 policy，并在 scene/snapshot 成功读取后写入统一 audit log。
...
- `uv run pytest tests/api/test_workline_routes.py tests/workline/test_plane_read_model_phase3.py -q`：21 passed
...
| ENG-021 | 🟡 部分完成 | `docs/contracts/observability-contract.md` 与契约测试已落地；本分支新增 `RuntimeObservabilityRegistry` 稳定 attributes 校验、observer 发射入口、`RuntimeOpenTelemetryBridge` exporter fan-out，并接入 WMS breaker transition 与 evidence persistence failure 运行时 instrumentation；其余 runtime/callback/device span 与生产 OpenTelemetry backend 接线仍待补齐 |
...
- [ ] WMS breaker/evidence、DeviceCommand ACK age、RuntimeInbox/Outbox 等关键指标已纳入观测口径。🟡 PR #73 已落地 typed evidence envelope 与 observability contract；本分支补齐 WMS breaker/evidence instrumentation、retention/archive 和 exporter bridge；DeviceCommand ACK age、RuntimeInbox/Outbox 生产指标接线仍待补齐。
