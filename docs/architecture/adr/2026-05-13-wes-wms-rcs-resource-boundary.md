...
Accepted - 2026-05-13；职责边界于 2026-09-22 收敛
...
1. WMS 是库存、预留、扣减、账务、SAP 同步和业务任务的唯一权威，并负责给出全部业务资格、来源、目标、
   优先级、业务路线、业务异常分类、替代来源、取消、恢复和业务终态结果。
2. WES 可以按自动化 SOP 的需要持久化已观察执行事实、过程快照与冲突证据；不得把投影升级为库存主账或物理资源准入依据。
...
5. `TransportTask` 拥有运输意图、内部关联、超时观测、结果证据和状态推进。ACK、已受理或已下发都不是完成事实；
   只有 Phase 4 Transport 合同定义、由 WMS 可靠异步回传的 TransportResult 才能推进等待中的运输动作。请求接收结果未知时保持原身份查询/幂等重提；时间流逝不生成物理终态。
6. WMS 普通业务事件与运输终态使用不同的类型化应用入口和 owner；两者可以复用同一 HTTP ingress 的信封、幂等和
   持久化能力。普通业务事件不能直接终结 `TransportTask`，也不能绕过具体对象 owner 修改执行对象或投影。
7. 满箱交换、生产发料、Pick_Fail、退料和入库确认都不得由 WES 本地改库存。WES 只记录执行事实并通过对应
   业务确认或 Transport 意图请求外部权威处理。
8. WES 解释当前 WMS 意图并结合已观察事实决定下一自动化动作：校验合同、关联、版本与时效，维护同一步骤身份及依赖。WES 不得重算或替换 WMS 给出的业务结果，也不得以预测容量、虚拟占用或历史任务裁决物理可执行性；设备资源由 ECS/RCS 接纳时裁决。缺少当前步骤必需的事实时只暂停其依赖动作。
...
| ECS 设备动作 | `DeviceCommand` + 第三方设备统一接口 | 不与 Transport 或 WMS 确认共用通用状态机 |
...
- Phase 4 Transport 合同为 `docs/contracts/transport-fulfillment-contract.md`；不得从厂商历史编号或旧 WMS integration
  实现推定目标 wire。
- Phase 3 不创建具体业务 API、业务 Port、`TransportTask`、`WmsConfirmation`、数据库模型、迁移、repository、evidence 表或通用熔断器。
- 任何实现计划若要求 WES 本地判断库存可用性、来源、目标、优先级、业务路线、业务异常分类、替代来源、空箱授权或业务
  终态，均违反本 ADR；不得通过新增本地规则、兼容模式或通用决策引擎绕过 WMS。WES 根据已获业务结果与现场证据决定
  物理等待、NG 路由、暂停和对账，不构成重算 WMS 业务结果。
