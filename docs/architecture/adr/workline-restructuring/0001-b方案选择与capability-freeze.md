...
3. **现有 typed port 包装不重建**：`wms_integration` 复用现有 `wms_integration`（实测 2,649 LOC，其中 typed_ports.py 609 行 + models/ports.py 151 行 = 760 行）；补全 5 套缺失 port（`WmsMasterDataPort` / `WmsDocumentPort` / `WmsFulfillmentPort` / `WmsEventPort` / `WmsReconciliationPort`）。
