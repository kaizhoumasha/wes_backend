...
**最后更新**: 2026年7月11日（feature/runtime-inbox-single-source-of-truth：RuntimeInbox 唯一事实源、五态 claim/fencing、三阶段 processor、旧 WorklineInbox 退役、PostgreSQL crash/benchmark 与 SLI）
...
| `services/runtime_inbox/` | RuntimeInbox 生产三阶段处理：context load、validation、orchestration、fenced write-back；旧批处理器已删除 | 🔧 架构核心 |
...
| `consumers/runtime_inbox_service.py` | RuntimeInboxService：ACK-before-processing、source-event 幂等接收、五态 claim/retry/fencing、payload conflict 409、DEAD_LETTER 人工重放审计 | 🔧 架构核心 |
