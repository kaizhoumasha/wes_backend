...
| R12 | 旧 `resource/*` 模型与服务曾保存到位、placement 和 Cell 占用事件 | **已按所有权审计删除；当前执行位置由 `PositionProjection` 承接** | 14 表审计、消费者切换与数据库删除见 `resource-model-ownership-audit.md`；WMS 主账不进入 WES | WorkLine 位置与执行投影测试 |
...
| R12 | **REMOVE — WMS Business Duplication** / 已实施 | 现场系统拥有到位事实，WMS 拥有库存和业务位置；WES 仅保留当前执行必要的 `PositionProjection` | 旧 Resource 服务无生产写入 owner，已删除 | 当前投影由已有 Transport/执行事实更新，不新增 Resource replay Worker |
...
| 旧策略/桥接（R14/R15）；R12 已完成 | `DeviceDispatchPolicy`、`ResourceWaitEvidence` 只有定义/导出或负载测试引用；R12 旧 Resource 服务已退出 | R12 的当前事实由 `PositionProjection` 及原执行 Evidence 承接；R14/R15 仍按各自调用链审查 |
...
| Projection：旧 Resource 投影已退役；`scan_flow` → `apply_device_position_result` | 当前执行位置由 Transport/设备权威事实更新 `PositionProjection`，WMS 库存另有权威 | **KEEP — Fact Projection** 仅限当前执行位置；不能因业务取消清空已确认到位。测试 owner 为 `test_scan_flow.py` 和 Transport 投影测试 |
