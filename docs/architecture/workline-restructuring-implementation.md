...
- [x] 北向下一次取料只由上一物料南向投放成功的 typed `COMMAND_RESULT` 解锁；扫码平台占用状态不作为取料准入条件，也不存在额外并发旁路
