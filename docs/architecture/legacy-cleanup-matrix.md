...
note: |
  逐入口数据在 legacy-cleanup-matrix.csv（676 条，由脚本生成，可复现）。
...
| --- | ---: |
| **total_entries** | **676** |
| phase4_carrier（承载 Phase 4 业务语义） | 124 |
| pending-review | 0 |
...
### 7.3 workline_plugins（22 entries，均为测试证据）
...
| `tests/workline_plugins/smt_sorting_inbound/test_smt_generated_plugin.py` | rebuild | 11 条 generated-only SMT 合同证据；由 `WorklinePluginDispatcher.dispatch` 与 PostgreSQL 闭环继续承载 |
...
| phase4 业务流程（124 entries） | phase4 | 粗分机/满箱交换/分拣机/SMT/NG 语义重建，须 characterization + contract test 先行 |
...
1. ✅ 每个旧入口都有且只有一个主策略（CSV 676 条，strategy 字段非空）
2. ✅ 标记是否承载 Phase 4 业务语义（phase4_carrier 字段，124 条）
