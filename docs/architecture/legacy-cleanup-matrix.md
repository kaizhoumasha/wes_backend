...
note: |
逐入口数据在 legacy-cleanup-matrix.csv（656 条，由脚本生成，可复现）。
...
### 7.2 workline_runtime（208 entries）

| 类别 | 处理 | 说明 |
| --- | --- | --- |
| `tests/workline_runtime/` | keep-contract / rebuild | 207 条 runtime / material-flow characterization 与合同测试，作为目标态能力闭合和 legacy 删除前的 blocking evidence；RuntimeInbox projection/repository 合同已入矩阵 |
...
| 风险项 | phase | 说明 |
| --- | --- | --- |
| 执行状态迁移（217 条执行状态语义；phase2 rebuild 总计 256 条） | phase2 | RuntimeInbox 已收敛到唯一事实源并通过崩溃重放验证；旧 `WorklineInbox` 仅保留历史审计说明，不再作为 characterization owner |
...
1. ✅ 每个旧入口都有且只有一个主策略（CSV 656 条，strategy 字段非空）
