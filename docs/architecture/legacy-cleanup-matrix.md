...
note: |
  逐入口数据在 legacy-cleanup-matrix.csv（599 条，由脚本生成，可复现）。
...
## 3. 汇总（截至 confirm_inbound typed EFFECT 收束 @ 2026-07-23）
...
### 7.2 workline_runtime（147 entries）

| 类别 | 处理 | 说明 |
| --- | --- | --- |
| `tests/workline_runtime/` | keep-contract / rebuild | 146 条 runtime / material-flow characterization 与合同测试，作为目标态能力闭合和 legacy 删除前的 blocking evidence；RuntimeInbox projection/repository 合同已入矩阵 |
...
| 风险项 | phase | 说明 |
| --- | --- | --- |
| 执行状态迁移（180 条执行状态语义；phase2 rebuild 总计 213 条） | phase2 | RuntimeInbox 已收敛到唯一事实源并通过崩溃重放验证；旧 `WorklineInbox` 仅保留历史审计说明，不再作为 characterization owner |
| phase4 业务流程（110 entries） | phase4 | 粗分机/满箱交换/分拣机/SMT/NG 语义重建，须 characterization + contract test 先行 |
