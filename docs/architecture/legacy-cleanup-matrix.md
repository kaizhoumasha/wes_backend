...
| 风险项 | phase | 说明 |
| --- | --- | --- |
| 执行状态迁移（196 条执行状态语义；phase2 rebuild 总计 189 条） | phase2 | RuntimeInbox 已收敛到唯一事实源并通过崩溃重放验证；旧 `WorklineInbox` 仅保留历史审计说明，不再作为 characterization owner |
...
1. ✅ 每个旧入口都有且只有一个主策略（CSV 611 条，strategy 字段非空）
