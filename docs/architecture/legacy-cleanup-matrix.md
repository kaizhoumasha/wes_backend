...
note: |
  逐入口数据在 legacy-cleanup-matrix.csv（593 条，由脚本生成，可复现）。
...
### 7.2 workline_runtime（174 entries）

| 类别 | 处理 | 说明 |
| --- | --- | --- |
| `tests/workline_runtime/` | keep-contract / rebuild | 173 条仍在核心测试树中的 runtime 合同；后续必须继续按 `CORE_REWRITE` 或 `LEGACY_DELETE` 收敛，不得承接具体插件行为 |
...
| 风险项 | phase | 说明 |
| --- | --- | --- |
| 执行状态迁移（229 条执行状态语义；phase2 rebuild 总计 251 条，phase2 全部 259 条） | phase2 | RuntimeInbox 已收敛到唯一事实源并通过崩溃重放验证；旧 `WorklineInbox` 仅保留历史审计说明，不再作为 characterization owner |
...
1. ✅ 每个旧入口都有且只有一个主策略（CSV 593 条，strategy 字段非空）
