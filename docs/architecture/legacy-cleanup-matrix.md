...
note: |
  逐入口数据在 legacy-cleanup-matrix.csv（650 条，由脚本生成，可复现）。
...
### 7.2 workline_runtime（203 entries）

| 类别 | 处理 | 说明 |
| --- | --- | --- |
| `tests/workline_runtime/` | keep-contract / rebuild | 202 条 runtime / material-flow characterization 与合同测试，作为目标态能力闭合和 legacy 删除前的 blocking evidence；RuntimeInbox projection/repository 合同已入矩阵 |
...
1. ✅ 每个旧入口都有且只有一个主策略（CSV 650 条，strategy 字段非空）
