...
**Arrival Fact ≠ Business Authorization ≠ Physical Execution Result。** SCAN1～SCAN4 各自上报本点实际到位与当前扫码事实；WMS 业务授权和 ECS 动作终态分别形成独立 Evidence。每个点先可靠记录到位，再根据本点箱码、当前 Requirement 和本料箱真正需要的前置确定结果决定 Action；缺少 SCAN2 不会自行否定允许直达 SCAN3 的现场路径。人工线 `ManualPickingPassage` 负责 INLET/SCAN1→SCAN3；首次 SCAN3 到位是前段结束事实，正常箱同事务交接给负责 SCAN3→OUTLET 的 `BinLineReturn`，不等待 SCAN3 Command 成功。SCAN4 首次到位是该 Return 的物理 FIFO admission 与顺序事实；匹配该点 `MOVE_FORWARD` 的 ECS `SUCCESS` 仅使它具备 `return_batch` 资格，不改变首次到位顺序。`return_batch` 仅消费 READY 候选并按首次到位顺序派发，历史 MOVE_PENDING 不阻塞其他已有确定到位结果的独立 READY；已进入 `RETURN_REQUESTED` 的箱不再参与待派发 FIFO，本箱原 Transport 明确 `ACCEPTED` 即完成执行权交接并闭合 Return，后续独立批次不等待 `SOURCE_PICKED`。发送未知、拒绝或仅创建本地请求不能证明交接；交接后 Transport 的超时、失败与位置未知仍由原身份继续履约/对账。`SOURCE_PICKED` 是可选物理证据，不能由 ACK 伪造；Return 闭合不声明 OUTLET 已物理清空，`TARGET_PLACED`/最终结果才更新相应物理位置与 Transport 终态。WES 不以本地 `received_at` 推断物理顺序，SCAN4 Event 时间的生成语义和重发稳定性仍以设备合同验收为准。
...
**这是所有业务设计、数据模型、流程编排、运行恢复和代码实现必须遵守的顶层架构红线。** WES 是执行编排系统，不是业务历史账本。过程数据只在相关业务与自动化执行尚未整体闭合时具有运行意义；闭合须同时覆盖业务执行、设备动作、Transport、Command、可靠交付以及必要的异常处理和补偿，不能只看单个 PickingTask 或 Action 的终态。

1. WES 正常运行不得依赖已闭合历史。`CLOSED` 之后的数据只用于追溯、审计、统计和故障诊断，不得用于新业务决策、当前运行恢复、新任务执行或当前状态推断。
2. 所有已闭合业务历史必须具备可清理性。清空全部 `CLOSED` / `PURGEABLE` 历史后，WES 仍须正常启动、恢复全部未闭合执行、继续与 RCS/ECS 协同，并接受和执行新任务。
3. 当前运行态必须自洽。运行输入只能来自当前有效配置、当前未闭合业务、当前执行事实、当前设备/现场事实，以及能明确证明必要性的当前运行投影；不得查询已闭合过程记录来补全当前状态。
4. 历史记录不得充当当前状态。不得以“最近一次历史记录”“上一 `plan_revision` 的最终状态”或“历史最后位置”推断当前业务或设备状态。
5. 现场状态区分 `OCCUPIED`、`EMPTY`、`UNKNOWN`。前两者均须有相应可信事实；记录不存在、历史已清理或查询无结果都属于 `UNKNOWN`，且 `UNKNOWN != EMPTY`。
6. Projection 只服务于当前未闭合执行所需判断，应尽可能由当前执行事实重建；不得扩展为 Rack、Bin、Material、Inventory 等 WMS 业务主账。
7. 同一物理对象可参与多个 Revision，但新 Revision 不得依赖上一 Revision 已闭合的过程数据才能正确运行。
8. 某业务数据若在相关业务和自动化执行整体闭合后仍被运行逻辑依赖，默认是架构异常；必须重新审查其权威来源、生命周期、当前状态模型、Projection 用途和 WMS/RCS/ECS 职责边界。长期保留的业务数据须证明属于配置或当前未闭合运行事实。
9. 模型设计和代码审查必须回答：“清空所有 `CLOSED` / `PURGEABLE` 历史业务数据后，WES 是否还能启动、恢复未闭合执行并接受新业务？”答案为否的设计原则上不得通过，除非证明被依赖的数据实际上属于配置或当前运行态。
...
保留不可变身份/正文、单任务领取、有效业务约束和明确管理停用；设备正常消息不得自动启用工作线。
`ESTOP_PRESSED` 不属于 WES callback 合同，急停记录、复位和恢复执行由 ECS 负责；WES 不保存该事件，保留原 DeviceCommand 身份和围栏并等待既有结果/对账终态。WES 核心保持独立部署和测试，不导入具体插件。

具体差异与五项外部能力见[接入核验表](../integration/third_party_integration_whitepaper.md)。
ECS 恢复排序、供应商同资源互斥/幂等期限及 WMS prepare/return_batch 确定响应机器纠正尚缺证据。
这些缺口阻止对应自动修订、业务完成或部署验收声明；不阻止已批准本地实现。
原现场事实、同任务完成依赖、显式停用及身份冲突校验不能因“无阻塞”被删除。
...
本项目不再定位为传统的 WMS，而是一个 **独立于现有企业级 WMS/SAP 的控制中台**。它旨在作为一个高可用、低延迟的智能化中间层，向上承接 ERP/WMS 的业务单据，向下协调自动化执行：当前交付只直接接入顶层 SPEC §3.1 已确认工作线所需的 ECS/WCS/视觉类作业设备；AGV/CTU/RCS 类搬运、交换、旋转任务统一提交现有 WMS 转发执行，WES 不直连 RCS。贴标、X-Ray、LCR 及机构件/SFC 协同属于本文保留的未来产品需求，不进入当前十四阶段实现与验收。
...
* **边界化接口**: 提供版本化 RESTful API 供上层系统调用；所有固定式设备供应商适配 WES 统一接口，搬运/交换/
  旋转类任务通过 WMS 接口转发。
* **核心域**:
  * **入库执行**: 码头收货、IQC 路由、上架策略执行。
  * **库存代理**: 采用 **按需动态查询 (On-Demand Query)** 模式，实时调用 WMS 接口获取库存数据，不维护本地库存副本。
  * **出库协同**: 接收 WMS 下发的 `PickingTask` 排队信息；WMS 接纳准备请求后按连续版本分批回调直接取料来源和五层来源货架面。
    五层货架到位后，WES 报告本批最大可取数量，WMS 再选择 Bin。Cell 在实际 Bin 到达 SCAN2 后创建，逐盘身份在设备扫码后晚绑定。
...
1. **独立部署能力 (Independent Deployment)**:
   ...
   * **网络依赖性**: 系统作业强依赖于上游 WMS 的在线状态。若与 SAP/Legacy WMS 断网，系统将执行以下策略:
     ...
     * **证据驱动恢复**: 网络恢复后继续可靠接收未决 `TransportTask` 的异步终态；已超过结果截止时间或提交结果未知的任务
       ...
       的对外幂等或关联字段后重提。取得终态证据后重新执行对象级准入；条件不满足、结果未知或证据冲突时保持暂停并
       进入人工对账。
2. **API 驱动架构 (API-Driven)**:
   * **API First**: 所有功能（包括前端 UI）均通过明确版本的 RESTful API 访问。
   * **边界合同**: 上层业务接口由 WMS 合同定义；所有设备供应商必须适配 WES 第三方设备统一接口。具体设备
...
下表是整体产品需求涉及的硬件清单，不等于当前交付清单。当前只实现顶层 SPEC §3.1 与十四阶段总控明确批准的工作线；
...
* **基础策略输入**: 从 WMS 获取其权威的供应商物料尺寸/厚度事实，保存工作线执行所需的最小版本化投影并绑定到
  WorkLine 当前配置，作为装箱算法输入；WES 不成为该主数据的权威所有者。
...
* **物理结构**：
  ...
  * **货位编码**: 单层移动料架每架 4 个货位，按 Excel 初始化协议使用 `A/B/C/D`
    顺时针编码；货位条码示例为 `NHW-1CLJ-0096-1C-1`。Transport 合同中的 `rack_face` 是 WMS/RCS 提供并由 WES 原样保存的
    普通不透明 string；按上下文可为 `null`，提供时不得为空字符串。不建立与本节 A/B 业务称谓、角度或货位字母的换算关系；`rack_slot_code=A|B|C|D`
...
* **通信协议**: 采用 `docs/integration/third_party_integration_whitepaper.md` 定义的 HTTP/JSON 统一接口；
...
* **未知结果处理**: 请求一旦可能被控制系统接收但未取得确定结果，命令进入 `TIMED_OUT`/远端结果未知状态，保留证据并暂停
  ...
  必须可靠保存并按原命令身份关联；不能覆盖已确认事实，也不阻断其他独立命令。
* **状态监控**: 所有供应商实现统一设备状态查询，顶层返回 `devices` 数组，每项分为只用于诊断的 `device` 元数据和
  `state`。运行模式映射为共享 `mode`，运行状态映射为共享 `status`；维护态属于 `mode=MAINTENANCE`，不得混入 `status`。
  `contract_key`、`contract_version` 由 WorkLine 当前绑定及命令自身在 WES 内部确定并冻结，不要求 Status 返回；无业务
  `MANUAL_DEBUG` 联调命令从受权限控制的 Swagger 请求冻结局域网 Endpoint，使用 WES 固定内部合同元数据；它不使用业务
  binding 或状态新鲜度合同，只校验请求身份和 `supported_commands` 静态能力。
  `contract_key`、`contract_version` 和回调幂等身份不进入白皮书 1.1 外部 wire。
  核心据此维护通用 `DeviceRuntimeProjection`；业务命令状态查询只用于诊断和对账，不授权发送，也不替代最终 CALLBACK。
...
* **WorkLine 插件、启动与分拣机设备边界**:
  ...
  * WorkLine 生命周期与料箱关联替换按 `docs/superpowers/specs/2026-09-06-bin-code-and-station-driven-flow-design.md`
    验收；本节表达目标约束，实际实现及验证进度以该 SPEC 的实施记录为准。
  * Fact、WMS WAIT 后继和 Transport outcome 保留原请求/执行身份，由所属 WorkLine 当前精确 `plugin_key + plugin_version` 处理；
    原义务未闭合时禁止停用和切换。已闭合消息重投返回原幂等结果，不重新驱动插件；违反准入的输入保存拒绝证据并报错，不建立跨插件接管机制。
  * START 是唯一启用入口，请求携带当前 WorkLine `version`。同一 WorkLine 锁和事务内检查版本、停用状态及准入条件，设置活动状态并推进版本；
    旧版本返回 `409`，不新增 request_id 或独立运行实体。客户端响应丢失或冲突后重读状态，不自动刷新版本再发启动。
  * 现场工作人员按停料和物理清线 SOP 操作后，系统在同一事务内检查 MaterialExecution、DeviceCommand、Transport、WMS 确认、待应用 Evidence 及插件业务等待/FIFO；未闭合执行阻断停用和配置切换。基础配置修改遵循相同检查。
    当前 ECS 状态接口没有能证明所有工作位为空的字段；设备 `IDLE`、无活动命令或 WES 查不到投影都不是 `EMPTY`。界面提示现场 SOP，位置投影用于诊断显示，不单独作为停用、插件切换和基础配置修改的放行或阻断依据。未闭合 Transport/Command 的未知结果仍由原执行对象阻断。不新增清线确认表、字段、接口、确认人/时间记录或自动排空编排；数据库事务不等待现场操作。
  * WorkLine 保存当前精确插件版本、配置及必要设备/位置绑定，运行期间禁止修改。DeviceCommand 和 Transport 保存自身身份、不可变请求和必要执行参数，
    不复制整份 WorkLine 配置。进程重启不生成新业务身份或自动更换插件版本。
  ...
  * 分拣作业启动必须同时满足业务需求、WorkLine READY、Station 业务 lease 空闲、单层货架 active 执行快照或 WMS 到位/授权回调；
    具体设备命令下发前只校验设备角色、冻结合同和请求参数，实际运行态、容量及物理互斥由 ECS 在接纳时裁决。
...
* **设备命令生命周期 (Device Command Lifecycle)**:
  ...
  * `MANUAL_DEBUG` 只用于无业务现场联调，不关联业务 WorkLine、设备主数据或执行对象；其 CALLBACK 只闭合命令与 evidence，
...
* **并发控制 (Concurrency Control)**:
  ...
  * 业务对象的总序及队列参数更新由 WMS 给出；设备状态只用于诊断，不代表核心或插件选择其他业务任务。
...
* **执行任务驱动**: 自动上架不新增 WMS `InboundTask`。WMS 根据冻结货架快照生成 `putaway_plan_id`，WES 在已激活
  `automatic_putaway` 的 WorkLine 中创建 `putaway_execution_id`；两者共同形成一次执行任务边界。目标五层货架、实际空 Bin 和
...
* **目标 Bin 供退**: WMS 只供给库存主账中存在可分配 Cell 的 Bin。正常 Bin 离开工作位后进入当前 WorkLine 物理缓存的 `RETURN_BUFFER` FIFO；WMS 为连续前缀分配当前 CTU 工作位货架面的任意合格精确空位，
  不要求原货架面。当前面无合格空位时 Bin 等待，不转 NG 或冲突；正常运行时只有新供给需求驱动换面、换架。停用或切换前由工作人员按现场流程停料，WorkLine 保持原插件及配置；共同 drain wire 已冻结，但本自动上架场景的停用/切换触发仍未实现，不得据此自动创建清场 Transport。全部系统义务及有效占用闭合且现场物理清线后才允许停用或切换。WES 只按现场缓存和 CTU 物理容量限流，不推断库存可用性。
* **Bin 物理准入**: WMS 冻结精确供给 Bin 后，WES 在 WorkLine 准入、对象锁和资源校验通过后创建 TransportTask；提交、接纳、失败和位置未知由原搬运对象负责。
  最终结果确认 Bin 成功到达冻结 HANDOFF_POSITION 且现场实扫 `bin_code` 匹配后，才允许该成员进入线内业务；不建立全程料箱执行实体。
  正常退箱的 FIFO 退出依据匹配成员的权威物理结果，WMS 位置报告独立可靠闭合。NG 立即结束正常业务，未决物理动作及有效占用继续可靠闭合，
  不新增 NG 出口报告或以人工取走作为正常业务完成门禁。下次供给使用当前 WMS 决定与新 Transport 成员，旧投影或条码不能单独授权。
...
* **业务完成**: 正常料盘只有在可靠 PUT 且 WMS 接纳位置 Fact 后才完成；Material NG、可靠空取和满箱交换按各自明确终态闭合。
  全部来源成员闭合后，由 WMS 根据不可变计划和已持久化事实裁决上架业务完成。
* **Bin/货架独立清场**: SCAN3 按已持久化 NG 处置分流，SCAN4 把正常 Bin 送入本线退料缓存。目标 Bin 退回、NG Bin 人工取走、
  来源货架搬离和相关 Transport 独立闭合，不阻塞业务完成；但全部物理义务闭合前，WorkLine 不得释放或切换插件。
...
4. 每条 WorkLine 只有一台 CTU，入站和退箱批次串行；五层来源成员按 `task_id + plan_revision + rack_id + rack_face` 区分，同一架面在不同 revision 可再次安排。
   ...
   CTU02 成功表示旋转后已返回工作位。投料未完成时仅在分段间隙最多一次机会式 `return_batch`，NO_BATCH 不阻断后续投料。
...
6. 设备取盘并扫描完整六合一码，`PkgID` 唯一标识当前料盘。WMS 返回 `ACCEPT | REJECT | WAIT`；`ACCEPT` 包含精确 SLOT 和需要的换面或换架模式，
   ...
   无法识别、方向错误或 WMS 明确业务 NG 是 BIN NG。空取、扫码不完整、可识别但非预期的 Bin、设备失败、业务等待和未知结果不属于 NG。CELL NG
   当前盘位置事实确认后关闭当前 Cell，Bin 到达 NG 出口时只补充最终位置；只有 BIN NG 影响整个 BinWorkExecution。未匹配物理
   Bin 可识别时保存预期/实际身份和位置证据并冻结，不送 NG、不替代或关闭预期成员；安全退回或对账 wire 获批前不得自动恢复。
...
9. WES 把每盘位置结果可靠提交给 WMS。当前任务的全部已接收明细都有处理结果，而且 WMS 已确认所有必须上报的逐盘结果后，WES
   携带 `last_applied_plan_revision` 请求 WMS 确认任务状态。尚无计划时该字段为 `0`。WMS 返回
   `COMPLETED | PLAN_REVISION_STALE | BUSINESS_IN_PROGRESS`，WES 不重复发送所有历史明细。`COMPLETED` 只表示当前任务结束，不表示
   订单需求全部满足。PickingTask 不设置 `FAILED` 状态。Bin、Rack、Transport 和工作线现场清理继续由各自流程处理。

自动出库的对象、不变量和验收场景以
`docs/superpowers/specs/2026-08-06-wes-outbound-operation-top-level-design.md` 为业务设计依据；具体 HTTP 请求和响应 JSON
...
8. WMS 不可用时停止新的 Task/Bin 推进，复用 Session `WAITING_EXTERNAL` 和 Outbox 可靠重试；不新增 WorkLine 状态，WorkLine 保持原插件及配置。WES 进程重启时校验启用 WorkLine 的插件精确版本仍在部署清单中，再从持久化证据恢复可靠处理；未知物理结果不得换身份盲目重发。

人工拣料当前流程以[人工出库交互要求](../contracts/wms-manual-outbound-picking-integration-requirements.md)及
[出库合同](../contracts/wms-outbound-picking-task-integration-requirements.md)的 RackCycle、FIFO 与 drain 章节为准；
人工操作不引入机械臂、Cell 或料盘字段，任务完成 drain 不改变其它尚未实现路径的边界。
...
* **执行对象要求 (Execution Object Requirements)**:
  ...
  `PositionProjection` 只表达 WES 活动管辖期内由可靠证据支撑的位置与队列诊断，不是 WMS 全局 Bin 位置镜像，也不作为跨任务执行授权。
  Transport 已接纳但尚无最终结果时保留原任务和成员事实；不建立跨任务资源 owner 或 gate。其他任务的匹配结果只更新其自身终态，
  若无法确认当前聚合位置，则将当前位置标记为未确认或未知，等待后续权威事实更新。
* **与现有 WMS 的协同机制 (Coordination with Existing WMS)**:
  ...
  * **能力**: 入库物理完成、自动出库逐项位置变化和自动出库整单完成分别形成可靠提交义务；SRS 不重复定义
    operation identity 或对外 DTO。
  * **幂等性保障**: WES 内部以闭集 operation identity 与 `dispatch_key` 唯一标识可靠义务；`dispatch_key` 不自动成为
    WMS 对外接口字段。Adapter 只发送逐 operation 获批的一个幂等或关联字段；未获 WMS 批准前不得声称远端原子去重。

  **4. 异常处理 (Exception Handling)：**

  * **场景**: 物理动作失败 (如: AGV 故障、装箱失败)。
  * **流程**:
    1. WES 根据权威失败证据更新对应 `MaterialExecution`、`DeviceCommand` 或 `TransportTask`；结果未知时保持对象级暂停，不伪造失败终态。
...
* **职责**: 上游 WMS 结果是业务权威；具体 WorkLine 插件校验其关联和物理可执行性后映射为封闭执行 Decision，核心分别推进 `MaterialExecution`、`DeviceCommand`、`TransportTask` 和 `WmsConfirmation`。
* **生命周期所有权**: 不建立统一 Task 状态机。每类执行对象只拥有自身状态、幂等键、终态证据和恢复规则，禁止用一个通用状态覆盖设备命令、搬运履约和 WMS 确认。
* **依赖表达**: 依赖通过明确的对象关联、权威终态证据和所属业务准入条件表达，不把位置投影当作跨任务授权，也不建设可配置 DAG、通用步骤表或跨工作线任务引擎。
* **并发边界**: 核心对每条 DeviceCommand 独立持久化和领取，不建立 `device_code` 活动占槽；ECS 在接纳时原子判断设备容量和物理互斥。
...
> **交付范围：未来需求。** 本节保留产品需求，但不属于当前顶层 SPEC §3.1 和十四阶段架构收敛的实现或验收范围。
...
> **交付范围：未来需求。** 本节不属于当前十四阶段计划的最终验收范围。LCR、X-Ray、贴标和退料工作线的真实
...
* **WES 机制**:
  1. **事实记录 (Evidence Persistence)**:
     * 每个关键边界分别保存物料执行或插件当前工位状态、位置投影、设备命令、外部义务和原始输入证据。
  ...
  3. **进程重启恢复 (Process Restart Recovery)**:
     * 校验各启用 WorkLine 的精确插件版本可用，从持久化请求、命令和 Evidence 恢复可靠处理，不创建新的运行身份。
...
* 人工线是固定无机械臂的 WorkLine，不是自动线的降级模式。
* 自动线故障、ECS 离线或紧急情况只停止受影响的自动执行并保留证据；WES 不在该线激活人工插件，不通过缺失设备角色或可空字段继续执行。
* 若业务需要后续人工处理，WMS 在原自动执行按物理边界确定收口后，按正常任务入口发布给独立人工线；不会热切同一运行中的 WorkLine 插件或迁移已接管的物理对象。
* PDA 只访问 WMS。来源物料、来源储位、目标 Bin/料格、人工扫描和库存事务由 WMS 授权并持久化；WES 只提供货架/Bin 位置事实与搬运能力。
...
* **WES 处理要求**:
  ...
  * **确认未发送**: 请求尚未离开 WES 时，只有逐 operation 外部合同明确批准安全重提且可靠执行对象显式
    `retryable=true`，对象才可保留原命令身份或内部 `dispatch_key`，映射到获批的唯一对外字段后重新提交；否则保持暂停
  ...
  * **可能已发送**: 已开始外部调用但未取得确定 ACK/终态时，必须标记为远端结果未知。设备命令进入
    ...
    `dispatch_key`，仅在逐 operation 合同明确批准安全重提且对象显式 `retryable=true` 时，才映射为唯一获批的对外字段后
  ...
  * 通信异常只暂停与未决命令、搬运任务或确认义务关联的物料执行或当前工位动作，不得无依据暂停整条工作线。
  * 自动出库任务中，物理结果未知只阻止依赖该结果的对象、设备和位置继续执行；它关联尚未完成的业务成员时，PickingTask
    保持执行中。PickingTask 已完成后的退箱或货架离场结果未知只保留原搬运身份、成员事实和告警，不回退任务，也不连带无依赖的对象、
    其他任务或工作线。
  ...
  * WMS 恢复后继续以相同事件身份重传未获成功 ACK 的 TransportResult；WES 对回调超期或提交结果未知的任务保持人工对账。
    ...
    安全重提且对象显式 `retryable=true` 时，才将内部原 `dispatch_key` 映射到唯一获批的对外字段后重提。取得终态证据后
...
* **现有 WMS 需提供的接口 (APIs Required from Existing WMS)**:
  ...
  operation、Method、Path、请求/响应 DTO 与错误合同由对应业务合同定义；WMS 转发的 RCS/AGV/CTU 对外接口由 Phase 4
  Transport 合同拥有。SRS 不维护第二份接口合同。
* **P9 WES 提供的接口能力 (API Capabilities Provided by P9 WES)**:

  **1. 执行状态查询 (Execution State Query)：**

  * 按 `MaterialExecution`、`DeviceCommand`、`TransportTask` 或 WMS 业务关联键查询当前状态、位置、未决义务和诊断证据。
...
* **幂等性设计 (Idempotency)**:
  ...
  * WES 可靠对象内部基于 `operation_identity + dispatch_key` 唯一标识业务义务；同一义务恢复必须保留原
    `dispatch_key`，但该键不自动进入对外接口。Adapter 只映射为逐 operation 获批的一个幂等或关联字段。
  * **内部幂等规则**:
  ...
  * **远端幂等规则**: 只实现对应 WMS 业务合同或 Phase 4 Transport 合同明确批准的对外字段、回显、冲突和安全重提语义；不得由内部规则扩张。
  * 避免网络重试导致的重复入库/出库。
* **事务失败后的处理 (Transaction Failure Handling)**:
  ...
  * 无论 WMS 确认请求可证明未发送还是可能已被接收，只有逐 operation 合同明确批准安全重提且 `WmsConfirmation`
    显式 `retryable=true` 时，才将内部原 `dispatch_key` 映射到唯一获批的对外字段后重提。条件不满足或结果仍未知时进入
    人工对账，不查询不存在的确认状态端点。
* **对账机制 (Reconciliation)**:
...
* **现有 WMS**: SAP 转发的业务数据、全部业务决定、出库 `PickingTask`、库存查询、库存预留和入库/出库确认。WES 的业务与库存数据都来自 WMS。
...
> 旋转提交及异步终态由 Phase 4 `Transport Port` 承接；首版不提供 WMS/RCS 状态查询或取消。WES 可以按
> `transport_task_id` 提供本地只读运维观察投影，但该投影不驱动轮询、控制或业务完成判定。WES 不锁定五层空箱、不交换库存属性、
> 不自动扣减库存，只保存执行事实、资源投影、回写证据和对账证据。精确接口字段必须由对应合同批准，不能从本 SRS 或旧实现推定。
...
* **定位**: WES 直连 ECS、视觉、贴标、X-Ray、LCR、打印机等固定式作业设备的统一接口层。
