...
* **核心域**:
  ...
  * **出库协同**: 接收 WMS 已完成业务拆解和来源分配的 `PickingTask`，协调自动线发料执行；WES 不计算波次。
...
* **L2 - 控制中台 (This System - P9 WES)**:
  * **职责**:
    ...
    * **业务插件 (Business Plugin)**: 每条 WorkLine 通过显式注册的代码插件实现本线局部路由、执行顺序和目标位置规则。
...
* **任务驱动**:
  ...
  1. **业务拆解**: WMS 根据 SAP 工单、出库单、波次、库存和产线需求形成执行级 `PickingTask`；WES 不读取或解释这些业务单据，也不生成波次。
  2. **任务接入**: `PickingTask` 必须提供精确来源 `reel_id`、物理来源位置和目标生产转运货架面/储位；结构错误整单拒绝。
  3. **任务调度**: WMS 提供优先级；WES 在工作位空闲后按优先级、下发时间和任务标识稳定选择下一任务，任务开始后不可抢占。
  4. **自动线执行**: 一台双面目标生产转运货架由一个任务独占，先完成目标 A 面再完成 B 面；来源货架以 A 面进入工作位，并按当前目标面执行。
  5. **异常与补料**: WES 隔离 NG 料箱并通知 WMS；替代库存和来源由 WMS 追加。无替代库存时，目标架进入缺料待补缓存并等待 WMS 明确恢复。
  6. **完成**: 全部料盘物理完成后，WES 通知 WMS“拣货完成”，并等待目标架移出工作位的物理证据。
...
* **现有 WMS**: SAP 转发的业务事实、出库 `PickingTask`、库存查询、库存预留、入库/出库确认——WES 的唯一业务/库存数据来源。
