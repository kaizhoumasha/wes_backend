...
* **核心域**:
  ...
  * **出库协同**: 接收 WMS 下发的多 Cell `PickingTask`；WMS 在任务启动时锁定 Cell，并在设备扫码后逐盘决定
    `PkgID`、物料资格、目标储位及 Cell 是否继续，WES 只协调作业期执行，不计算波次或选择库存来源。
...
* **任务驱动**:
  ...
  1. **业务拆解**: WMS 根据 SAP 工单、出库单、波次、库存和产线需求形成多 Cell `PickingTask`；任务冻结必须执行的
     `BIN_CELL` 和可选单盘 `RACK_SLOT`，不预绑定 `PkgID`、SixInOne、目标转运货架或目标储位。WES 不读取或解释
     业务单据，也不生成波次。
  2. **启动授权**: WMS 原子锁定任务 Cell 集合并提供锁代际、目标架容量/面顺序及所需货架搬运目标；WES 不自行选料、
     增删 Cell、分配库存或改变来源。
  3. **并行搬运**: 目标空架、可选退料架和第一台五层货架可并行调度；指定五层货架由 AGV 送达 CTU 作业位并取得可靠
     到位证据后，CTU 才按冻结成员批次向本线投箱。只要入料缓存有可用准入位置即可继续投箱，不等待当前料箱完成。
  4. **三段缓存**: 滚筒线按入料缓存、工作位缓存和退料缓存运行；工作位缓存容量包含机械臂实际工作位。容量从逐位置
     投影聚合，SCAN1/SCAN3 负责正常/NG 路由，任一段满载只向直接上游施加背压。
  5. **逐盘晚绑定**: 机械臂从当前 Cell 顶部取盘并扫码；WES 持久化扫码证据后请求 WMS 返回唯一 `PkgID`、SixInOne、
     物料接受/拒绝、当前目标面内的唯一目标储位及 `CONTINUE | CELL_DONE`。WES 不本地判断物料资格或是否继续。
  6. **独立完成**: `CellExecution` 在 WMS 明确结束且最后一盘物理闭合后完成；全部 Cell 完成即 `PickingTask` 完成。
     Rack、Bin、AGV/CTU 和工作位清场由独立执行对象继续闭环，不进入 PickingTask 完成聚合；工作线是否允许下一任务进入
     由当前 `LineRunEpoch` 下的设备/位置投影和活动对象共同判断，不新建通用 Session。

详细对象、状态、双面顺序、LIFO、取消和恢复规则以
`docs/superpowers/specs/2026-08-06-wes-outbound-operation-top-level-design.md` 为准。
...
* **核心接口**:
  ...
  * 自动出库由 WMS 下发多 Cell `PickingTask`；创建、启动锁、优先级更新、逐盘扫码决定、恢复和取消的
    method/path/DTO 以独立 inbound wire 合同为准，WES 不接收生产工单生成波次。
