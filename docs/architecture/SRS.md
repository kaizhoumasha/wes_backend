...
* **业务用途**：
  ...
  * **满箱交换**: 支持业务按 WMS 决定，以一个普通 CTU `MOVE` 批次搬运具有相反来源/目标的料箱。
* **典型场景**：
...
* **WMS 决策、WES 执行逻辑**:
  ...
  2. **满箱交换执行 (Full Exchange Execution)**:
     ...
     * WES 根据源单层货架和释放快照创建普通 `MOVE` 类型的 `TransportTask` 并提交给 WMS；Transport 只保存相反
       来源/目标，不接收满箱/空箱分类，也不定义 `EXCHANGE` action。WMS 负责转发 RCS/CTU 执行。
