...
一次客户交付可以同时包含厂商 Adapter 和 WorkLine 执行插件，但二者是并列的独立包：Adapter 拥有厂商 wire 合同和
标准化映射，插件只消费角色化输入与 WMS 封闭业务结果，并拥有执行 Decision。任一包都不得以自己的测试替代另一包或
核心基础能力验收。
...
设备内部步骤、坐标、机械互锁和安全逻辑属于 ECS/PLC。插件只把 WMS 已给出的业务结果映射为逻辑设备动作，
...
插件不读取厂家原始 JSON，也不声明厂商协议依赖。新增厂家时，不应修改最小执行内核或执行插件映射。
...
| WMS 业务结果 | 当前步骤的授权、来源、目标、优先级、路线或处置；必须是封闭 operation-specific result |
...
- WMS 业务结果缺失、过期、矛盾或物理不可执行时 fail closed；插件不得选择替代业务方案。
...
| 插件逻辑 | 标准化输入与 WMS 结果下的执行 Decision、设备忙、物理冲突、模式不匹配、业务 NG 执行、硬件故障、依赖暂停 |
...
标准化映射；插件测试只覆盖 WMS 结果到执行 Decision 的映射和对象推进。三类测试不得互相替代或复制。
...
- [ ] WMS 业务 NG 结果、硬件故障、依赖暂停和人工清线语义明确分离，插件不本地改判。
