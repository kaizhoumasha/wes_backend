...
基础配置与业务装配分别保存：基础配置维护本线物理设备归属、静态货架工作位及工作位关联的物理设备；
工作位容量表示可容纳的货架数量，不表示货架层数。业务装配选择已安装插件，并将其声明的设备角色绑定到本线已有设备。
切换或解除插件不会删除工作位，也不会解除物理设备归属；移除被插件引用的设备前，须先解除对应角色绑定。
两类保存共用 WorkLine 版本校验和停用、未完成负载及安全事件检查；保存一类配置后，另一类编辑应重新读取最新版本。
基础工作位定义不自动成为插件的位置角色声明，也不改变插件已有的位置与 START 约定。

业务装配中的插件配置只保存 `device_bindings`，由通用界面将后端插件声明的角色绑定到本线实体设备。
