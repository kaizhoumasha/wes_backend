# wms\_rcs\_interface\_requirements

# MCS 对 WMS/RCS 接口需求分析

> **版本**: 3\.0
**日期**: 2026\-03\-14
**依据**: SRS\.md, third\_party\_integration\_whitepaper\.md, workline\_plugin\_architecture\_design\.md
...
|**MCS**|执行协调、策略引擎、设备编排|中间层|
|**RCS**|AGV/CTU 调度、路径规划|由 WMS 统一调度|

### 1\.2 集成边界（SRS\.md §3 本阶段集成边界）

1. **RCS 调度**: 仍由 WMS 统一调度。MCS 生成搬运需求并提交给 WMS，由 WMS 调用 RCS 并将结果/事件回传 MCS。

2. **PDA 交互**: PDA 仅对接 WMS 应用。MCS 如需感知 PDA 结果/事件，由 WMS 推送/同步。

3. **自动化设备**: 所有自动化设备只通过 MCS 接入，WMS 不直连设备。

4. **标签打印**: MCS 生成打印模板/ZPL。若为自动打印设备则由 MCS 下发；若为人工/非自动打印，则 WMS 获取模板后完成打印并回执结果。

### 1\.3 核心架构原则（遵循 workline\_plugin\_architecture\_design\.md）

**WMS 作为"外部系统"调用 ****MCS**** 时，必须遵循白皮书定义的标准回调接口：**
...
- **MCS**** 不同步基础数据**: 所有需要用到的基础数据向 WMS 请求

- **WMS 是库存唯一真实源**: 所有库存变动必须在 WMS 端事务提交成功后，物理动作方可视为完成

- **不过度设计**: MCS 只关注需要它完成的功能

- **统一入口**: 所有外部输入通过 `callback/event` 和 `callback/result` 进入 MCS

---

## 2\. 基础数据需求

> **说明**: MCS 不同步基础数据，按需动态查询 WMS。所有接口遵循 RESTful 规范，采用异步回调机制。

### 2\.1 物料主数据 \(Material Master Data\)

**业务背景**: MCS 在装箱校验、分箱算法、发料决策等场景需要实时获取物料属性信息。
...
```HTTP
GET /api/wms/materials/CAP001|Vendor001 HTTP/1.1
Host: wms-server
Authorization: Bearer <MCS_TOKEN>
```
...
- `standard_thickness` 允许 ±10% 偏差，MCS 在视觉识别后校验实际厚度
...
```HTTP
GET /api/wms/materials?ids=CAP001|Vendor001,RES002|Vendor002,IC003|Vendor003 HTTP/1.1
Host: wms-server
Authorization: Bearer <MCS_TOKEN>
```
...
**业务背景**: MCS 需要查询区域和地码信息用于设备归属解析、货架位置管理、搬运任务起点/终点定义。
...
~~        "z": 0~~
~~      },~~
      "rack_capacity": 4,                      // 区域可放置货架数
      "current_rack_count": 0,                 // 当前货架数
      "status": "AVAILABLE",                   // 状态：AVAILABLE/OCCUPIED/DISABLED
      "allowed_rack_types": ["SINGLE_LAYER", "FIVE_LAYER"]   // 区域允许的货架类型，单层、五层、转运、退货....
    },
    {
...
- `coordinates` 用于 RCS 导航，MCS 仅作为透传，不解析具体坐标值

- `rack_capacity` 用于 MCS 判断区域是否可容纳新货架
...
**业务背景**: MCS 需要查询货架和料箱属性用于分箱决策、混合入库策略、发料波次计算。
...
```JSON
{
  ...
  "data": {
    ...
    "capacity": {
      "total_slots": 4,                 // 总储位数 (单层=4, 五层=20)，即货架能放多少个箱子
      "occupied_slots": 0,              // 已占用储位数，已经放了多少个箱子
...
~~    },~~
    "side_a": {                         // A 面储位 (仅当 has_a_side=true)
      // 每一层放了哪些箱子
      "layer_1": [{"bin_id": null, "status": "EMPTY"},{"bin_id": null, "status": "EMPTY"}],
...
- `status=LOCKED` 表示货架正在搬运中，MCS 不得下发新的操作指令
  ...
  "data": {
    ...
    "capacity": {
      ...
      "cells": [                        // 各料箱内料格详情
        {
          "cell_id": "SLOT-01",
          "cell_type": "7inch",         // 储位类型：7inch/13inch/15inch/LARGE
          "status": "OCCUPIED",
...
|项目|说明|
|---|---|
|**接口地址**<br>|`GET /api/wms/racks?type={rack_type}&status={status}&zone={zone_code}`|
|**请求方法**|GET|
...
```JSON
{
  ...
  "data": [
    {
      "rack_id": "RACK-F001",        // 货架编码 (主键)
      "rack_type": "FIVE_LAYER",     // 货架类型
      "status": "EMPTY",             // 货架状态
      "current_location": "SMT_STORAGE_A01",   // 当前位置
      "side_a_empty_slots": 5,            // A 面空储位数
      "side_b_empty_slots": 5,            // B 面空储位数
      "accessibility_score": 0.9  // 访问便捷度 (0-1)，用于冷热区优化，暂时不要，后续优化
    },
    {
      "rack_id": "RACK-F002",
...
- MCS 可对查询结果进行短时缓存 \(TTL ≤ 30 秒\)，减少 API 调用频率
...
**业务背景**: MCS 在 PKG 校验、装箱任务创建时需要验证 GRN 存在性和物料归属关系。
...
```JSON
{
  ...
  "data": {
    ...
    "po_item": "001",                   // 行项目号
    "status": "PARTIAL_RECEIVED",       // 状态：CREATED/PARTIAL_RECEIVED/COMPLETED
...
- `status` 用于 MCS 判断 GRN 是否可继续收货：`COMPLETED/CLOSED` 不得再绑定
    ...
    "packages": [
      {
        ...
        "current_location": {
          "rack_id": "RACK-001",     // 所在货架
          "bin_id": "BIN-001",       // 所在料箱 
          "slot_id": "SLOT-01"       // 所在料格
        },
...
- `pkg_code` 是六合一码：`Material-Qty-Serial-LC-DC-Vendor`，MCS 需解析并校验各字段

- `status=KITTED` 表示已完成装箱，装到了单层货架，`status=STORED` 表示已上架五层货架

- `summary` 用于 MCS 判断 GRN 收货进度

---

### 2\.5 库存查询 \(Inventory Query\)

**业务背景**: MCS 采用纯代理模式，不维护库存主数据，所有库存查询实时透传 WMS。
...
- MCS 可对查询结果进行短时缓存 \(TTL ≤ 30 秒\)，但不改变"WMS 为库存主数据源"的定位
...
```JSON
{
  ...
  "data": {
    "rack_type": "FIVE_LAYER",   // 货架类型
    "status_filter": "EMPTY",    // 对料箱的过渡状态
    "total_empty_bins": 50,      // 总的空料箱数
    "bins": [                    // 料箱信息
      {
        "bin_id": "BIN-F001-A1",  // 料箱 ID
        "rack_id": "RACK-F001",   // 所在货架
        "side": "A",              // 货架面
        "layer": 1,               // 层
        "slot": "SLOT-01",        // 位
...
- MCS 需对查询结果进行本地缓存，避免短时间内重复调用相同参数的接口

---

## 3\. WMS → MCS 标准回调接口（遵循白皮书规范）

> **重要**: WMS 作为"外部系统"调用 MCS，必须使用白皮书定义的标准回调接口

### 3\.1 统一入口：POST /api/v1/callback/event

**设计原则**（遵循 workline\_plugin\_architecture\_design\.md §6\.1）:
...
```JSON
{
  ...
  "timestamp": 1702627300000,
  "data": {
    "rack_id": "RACK-001",                 // 货架号
    "rack_type": "SINGLE_LAYER",           // 货架类型
    "arrival_location": "KITTING_AREA",
...
- MCS 根据 `(device_code + event_type + timestamp + request_id)` 进行幂等检查
...
> **注意**: 这是 MCS 主动调用 WMS 的接口，不属于白皮书回调接口范畴

### 4\.1 空架补给请求

**接口**: `POST /api/MCS/rack-supply-request`
...
**接口**: `POST /api/MCS/transport-request`
...
## 5\. 架构演进路径（遵循 workline\_plugin\_architecture\_design\.md）
...
sequenceDiagram
    participant WMS
    participant MCS_Callback as MCS /callback/event
    participant MCS_Inbox as MCS WorklineInbox
    participant MCS_Orchestrator as MCS Orchestrator
    participant ECS
    participant RCS

    Note over WMS,MCS_Callback: 1. GRN 单据接入（通过标准回调接口）
    WMS->>MCS_Callback: POST /api/v1/callback/event {event_type: "WMS_GRN_RECEIVED", ...}
    MCS_Callback->>MCS_Inbox: 写入 Inbox (kind=EXTERNAL_CALLBACK)
    MCS_Callback-->>WMS: 200 OK (立即返回)
    MCS_Orchestrator->>MCS_Inbox: 消费 Inbox
    MCS_Orchestrator->>MCS_Orchestrator: 解析 -> 创建 Session

    Note over WMS,MCS_Callback: 2. 栈板到达通知
    WMS->>MCS_Callback: POST /api/v1/callback/event
{event_type: "WMS_PALLET_ARRIVED", ...}
    MCS_Callback-->>WMS: 200 OK

    Note over MCS_Orchestrator,WMS: 3. 空架补给请求
    MCS_Orchestrator->>WMS: POST /api/MCS/rack-supply-request
    WMS->>RCS: 调度 AGV
    RCS-->>WMS: 完成

    Note over WMS,MCS_Callback: 4. 货架到达通知
    WMS->>MCS_Callback: POST /api/v1/callback/event
{event_type: "WMS_RACK_ARRIVED", ...}
    MCS_Callback-->>WMS: 200 OK

    Note over ECS,MCS_Callback: 5. 设备事件（视觉扫描）
    ECS->>MCS_Callback: POST /api/v1/callback/event
{event_type: "SCAN_COMPLETED", ...}
    MCS_Callback-->>ECS: 200 OK

    Note over MCS_Orchestrator,WMS: 6. PKG 绑定通知
    MCS_Orchestrator->>WMS: POST /api/wms/kitting/pkg-binding

    Note over MCS_Orchestrator,WMS: 7. 满架搬运请求
    MCS_Orchestrator->>WMS: POST /api/MCS/transport-request
    WMS->>RCS: 调度 AGV
    RCS-->>WMS: 完成

    Note over WMS,MCS_Callback: 8. 搬运完成通知
    WMS->>MCS_Callback: POST /api/v1/callback/event
{event_type: "WMS_TRANSPORT_COMPLETED", ...}
```
...
|接口|方法|用途|优先级|
|---|---|---|---|
|`/api/``MCS``/rack-supply-request`|POST|空架补给请求|P0|
|`/api/``MCS``/transport-request`|POST|满架搬运请求|P0|
...
根据 workline\_plugin\_architecture\_design\.md：
...
## 10\. 附录：WMS/MCS 开发人员参考

### 10\.1 WMS 侧开发要点

**事件上报要求**:

1. 所有调用 MCS `callback/event` 和 `callback/result` 的请求必须携带唯一 `request_id`

2. `request_id` 格式建议：`REQ-{YYYYMMDD}-{SEQ}`，如 `REQ-20260314-001`

3. 若 MCS 返回 200 OK，表示事件已接收；若返回 4xx/5xx，WMS 需记录失败并重试

4. 重复事件会被 MCS 拦截，WMS 不应依赖重复上报来保证可靠性

**数据一致性**:

- WMS 是库存唯一真实源，MCS 所有库存查询都实时透传 WMS

- WMS 需在库存变动事务提交成功后，方可通知 MCS 执行后续动作

- 若 WMS 与 MCS 断网，MCS 将暂停所有涉及库存变动的任务
...
3. 若 WMS 接口超时或返回 5xx，MCS 需触发熔断并告警
