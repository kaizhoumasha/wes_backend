...
本目录保留硬件供应商提供的原始协议、联调资料以及便于检索的派生转写。所有文件均作为设备合同附录或 WorkLine
插件的外部输入保留，不是 WES 核心架构真源。

## 原始资料

- 厂商 PDF、图纸和原始流程材料保持原貌。
- 资料较旧或与实现不一致时，不修改原文，也不因架构收敛而归档。
- 当前差异由第三方设备统一接口白皮书、设备合同附录和供应商一致性验收处理。
...
- 派生资料中的字段归一化、插件名称以及 Session、Outbox、Inbox、Timeline、Hold 等实现描述只用于偏差识别，
  不得约束顶层 SPEC、设备统一接口或插件设计。

## 所有权

- 共享传输和基础传输错误由 WES 核心合同验证；第三方固定式设备在纯局域网内统一使用白皮书协议，不增加供应商私有
  认证、路径或 DTO 分支。当前 WMS outbound 固定为 `NONE`。
...
- 供应商 ECS/网关负责把内部 DTO、Payload、原始码和事件/命令收敛为 WES 统一接口，并独立通过设备合同附录一致性验收。
