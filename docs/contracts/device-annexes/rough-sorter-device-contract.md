...
WorkLine 的 `config.device_bindings` 是唯一角色到实际 `device_code` 的映射。START 校验本线设备、Endpoint、合同及实时能力，
将必要设备合同和位置绑定保存为 WorkLine 当前配置；不创建运行代际、配置快照副本或拓扑摘要框架。运行期间禁止修改插件及配置。
Device 主数据只保存物理身份，不保存业务角色。

| 绑定项 | 规则 |
| --- | --- |
| `device_role` | 每个 WorkLine 三个角色各一个绑定；只在 `config.device_bindings` 中维护 |
| `device_code` | 全厂唯一；不能用 Endpoint 数量替代设备身份 |
| Endpoint Base URL | 来自可派发 Device；必须为非空局域网 HTTP origin，固定路径不进入配置 |
| `contract_key`、`contract_version=1.0` | 插件声明；实时状态接口验证命令能力，测量角色还必须支持 `SCAN_COMPLETED` |
| `status_max_age_ms`、`command_timeout_ms` | 插件固定为 10,000 ms 和 30,000 ms；供基础派发读取，不进入命令 `params` |

DeviceCommand 创建时冻结执行必需的目标、Endpoint、合同和时限。可靠重试复用该命令自身保存的内容，不回读新配置改写旧命令，
也不复制整份 WorkLine 配置。不保存 `rough_sorter` 私有配置子树、动态版本登记或未被执行逻辑消费的参数。
...
粗分纯 Decision 层只声明角色、合同和业务处理逻辑，不读取或保存 Endpoint。START 检查三个可派发角色的 Device Endpoint 非空。
业务诊断读取当前 WorkLine 及原命令、请求、Evidence；当前配置不冒充历史运行快照。
修改插件或绑定前，现场工作人员完成停料及物理清线，系统在同一 WorkLine 事务内确认原任务、设备命令、Transport、WMS 义务、
待应用 Evidence、插件业务等待及有效占用均已闭合，然后才允许停用和配置修改。重新启动携带当前 WorkLine `version`；
旧版本请求返回冲突，不新增 `request_id`、清线确认记录、插件注册表或 Endpoint 注册表。
...
| 字段 | JSON 类型 | 必填 | 约束 |
| --- | --- | --- | --- |
| `location_id` | string | 是 | 当前 WorkLine 配置的稳定逻辑位置 |
...
当前业务拒绝指定且当前 WorkLine 配置的位置。设备坐标、供应商 `location_id` 别名和机械参数不得进入 WES 合同。
...
| 供应商位置/坐标 | 映射为当前 WorkLine 配置的 `location_id` 和第 5 节类型；私有值不外泄 |
