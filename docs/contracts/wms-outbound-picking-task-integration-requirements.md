...
| 系统 | 必须负责 | 不应负责 |
| --- | --- | --- |
| WMS | 任务版本、来源 Cell 原子锁、库存与物料资格、扫码到 `PkgID` 的唯一识别、目标储位、`CONTINUE / CELL_DONE`、业务取消与恢复 | 机械臂动作、滚筒线缓存、现场位置投影或设备互锁 |
...
4. 机械臂从指定 Cell 取盘并放到扫码台；设备扫码字段必须足以让 WMS 唯一确定该盘 `PkgID`。
5. WES 提交扫码证据，WMS 返回该盘身份、业务决定、目标储位以及 `CONTINUE` 或 `CELL_DONE`。
6. WES 按决定完成物理动作，并逐盘向 WMS 提交位置变化事实。
7. WES 仅在全部 `CellExecution` 完成后将 `PickingTask` 标记为完成；任务完成通知不得越过尚未被 WMS 接受的逐盘事实。
8. Rack、Bin、AGV、CTU 和工作线清场继续独立闭环，不参与 PickingTask 完成判断。
...
| WMS → WES | 任务控制 | 只更新未开始任务优先级；明确暂停、恢复和取消决定 |
| WES → WMS | 请求逐盘决定 | 使用任务、Cell、锁代际和扫码证据关联一次实际取盘 |
| WMS → WES | 返回逐盘决定 | 给出唯一 `PkgID`、完整 SixInOne、接受/拒绝/等待、目标储位和 Cell 后续动作 |
...
启动后的 Cell 集合不可静默增删或替换。需要变更时，WMS 应取消当前任务并创建新版本或新任务。
...
- WMS 识别出的 `PkgID`、完整 SixInOne 和权威版本。
- `ACCEPT | REJECT | WAIT` 之一；不得用空值或 HTTP 状态代替业务决定。
- `ACCEPT` 时必须给出当前目标面内唯一且可用的目标货架、目标面和目标储位。
- `CONTINUE | CELL_DONE` 之一，表示当前盘闭合后是否允许从同一 Cell 创建下一次取盘。
- 拒绝、等待或人工处理时的封闭原因类别和后续动作要求。

`CELL_DONE` 只表示“不再从该 Cell 取下一盘”，不表示当前已扫码料盘已经完成物理放置。

### 5.5 逐盘与任务事实

逐盘位置变化事实至少包含唯一 `fact_id`、任务/Cell/`PkgID` 关联、来源、目标或异常落点、设备终态证据、发生时间和事实版本。
