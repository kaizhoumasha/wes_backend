# WMS 北向 33 项 Operation 合同

> 状态：Decision A 已批准并修订；初稿覆盖 16 项的 path/业务字段作为 wire 基线，E02 method 已批准改为 `POST`；完整
> 出库顶层设计已删除旧 Q10–Q13，并新增 E17/E18 语义能力；完整字段矩阵、其余待定项，以及
> E08–E14 status/状态闭集/关联键/幂等承诺仍待完成
> wire 主真源：本文
...
- 33 项 identity 和 completion owner 作为当前目标 surface。初稿覆盖的 Q01–Q10、
  E01/E02/E05/E07/E08/E09 共 16 项以初稿 path 和业务字段为 wire 基线；method 采用本文当前矩阵，其中 E02 已批准改为
  `POST`。当前架构同时剔除旧认证、重试、缓存、分页和旧生命周期语义，其余 17 项仍待 WMS/业务方补齐。
- proposed surface 中 15 项 QUERY 无副作用：Q01–Q14 使用 GET，Q15 使用无副作用 POST JSON body。
...
- 测试态 conformance harness 必须核对 33 个能力模块、端口方法、Gateway 绑定和共享错误映射；该 harness
  不得进入生产装配。
...
它概念性覆盖 Q01–Q10 和 E01/E02/E05/E07/E08/E09，但不是完整 33 项合同，也不是当前架构真源。
...
| 未覆盖的 operation | 必须由 WMS/业务方补充或明确删除，不得从旧代码猜测 |
...
| Q10 | `GET /api/wms/inventory/query` | 保留 | 完整 query/result 字段和拒绝码 |
...
初稿未覆盖 Q11–Q15、E03–E04、E06、E10–E18。它们的 operation identity、completion owner和业务目标暂保留，
但 method/path/DTO/拒绝码仍是待批准提案，不得进入实现。
...
Phase 3 Task 1 必须建立 33 项逐项差异矩阵，记录初稿 method/path/字段/错误码、本文 proposed 合同、最终裁决和缺失输入，
...
初稿覆盖 16 项的 path/业务字段及当前批准 method 已按 Decision A 冻结；完整字段矩阵、其余 17 项裁决，以及 E08–E14
...
下表中 Q01–Q10、E01/E02/E05/E07/E08/E09 已采用初稿 path/业务字段及当前批准 method；其余 17 项仍是待批准提案。
...
| Q10 | `wms.inventory.query_inventory@v1` | GET | `/api/wms/inventory/query` | `query_inventory.py` |
| Q11 | `wms.inventory.get_reservation@v1` | GET | `/api/wms/inventory/reservations/{reservation_id}` | `get_reservation.py` |
| Q12 | `wms.reconciliation.check_bin_drift@v1` | GET | `/api/wms/reconciliation/bin-drift` | `check_bin_drift.py` |
| Q13 | `wms.reconciliation.check_rack_drift@v1` | GET | `/api/wms/reconciliation/rack-drift` | `check_rack_drift.py` |
| Q14 | `wms.reconciliation.check_full_drift@v1` | GET | `/api/wms/reconciliation/full-drift` | `check_full_drift.py` |
| Q15 | `wms.document.validate_rough_sorter_admission@v1` | POST | `/api/wms/documents/rough-sorter-admission/validate` | `validate_rough_sorter_admission.py` |
...
| E17 | `wms.fulfillment.report_picking_source_ng@v1` | 待批准 | 待批准 | `SYNC_RESULT` | `WmsConfirmation` | `report_picking_source_ng.py` |
| E18 | `wms.fulfillment.confirm_picking_completed@v1` | 待批准 | 待批准 | `SYNC_RESULT` | `WmsConfirmation` | `confirm_picking_completed.py` |
...
Q15 未被初稿覆盖。当前业务目标提案要求 request 包含 raw code、canonical
...
Q15 的 WES 业务边界不创建绑定、预留、扣减或收货进度。合同获批后，首次有效结论在设备下发前成为 WES admission fact，
...
- E11–E13 未被初稿覆盖。当前 WES 业务目标分别是：交换任务提交满箱、原储位、冻结 occupancy 与约束；入线批次冻结
  ...
  method/path/DTO/拒绝码必须在其余 17 项裁决中确认后才能实现。
...
E17 只报告已确认的来源 NG/身份异常事实，不替 WMS 选择替代库存。E18 只在全部任务项完成、目标架一致且无未决物理动作时
报告 `PickingTask` 拣货完成。二者的 method/path、DTO、幂等和拒绝码必须由 Phase 3 Task 1 与 WMS 冻结。

WMS 创建、更新优先级、追加来源、恢复和取消 `PickingTask` 属于 WMS → WES inbound 合同，不通过 Q10–Q13 拉取。
