...
| `reason_code` | 仅 `REJECTED` 为稳定、可枚举的非空 string；所有其它状态必须为 `null`。 |
...
`REJECTED` 不得伪造成功结果。完成态幂等重放必须逐字段保留原 `source_version`、`provider_reference` 和 typed result。
...
| 保留期边界/响应体上限 | 用可控时钟证明保留期内不重新生效，并拒绝超过承诺最大响应体的 wire body。 |
