...
业务完成判定。唯一写入例外是数据可丢弃联调环境中的定向清理：操作员已在 WMS/RCS 侧确认没有真实任务、物理动作和在途
callback 后，可以删除一个 `RECONCILING` 且没有 `TransportEvidence`、`outcome` 的本地联调残留。该动作不是远端取消或重试，
不得向 WMS/RCS 发送请求，也不得改变已有搬运事实。
...
`arrival_face` 为闭集 `A | B`，是 WMS/RCS 对货架实际到达工作面的权威事实，不是搬运提交的目标指令。缺少应有
...
直到匹配的权威确定结果完成消歧。唯一例外是第 1 节定义的联调残留定向清理：事务锁定任务并再次确认没有 Evidence/outcome 后，
随任务聚合删除其绑定；晚到 callback 仍按既有 missing-task Evidence 合同保留为 `CONFLICT`，不得静默丢弃。资源冲突在创建阶段
失败关闭，不等待 RCS 再拒绝。
