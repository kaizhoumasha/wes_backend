...
> 2026-09-11 目标补充：[WES 无阻塞执行与自动事实归集设计](2026-09-11-wes-nonblocking-execution-design.md)进一步明确新事件不等待旧对象业务对账。本文全程实体退役等有效范围保留；跨点处置关联与本站等待规则按新目标逐项评审后同步合同，不能直接删除必要信息校验或声称已经实现。
...
- Transport 创建可发送义务前，由 WorkLine 与所属业务确认该动作、对象归属及冻结成员合法；不使用旧位置投影或跨任务资源 owner
  作为执行授权。清场动作按原插件职责执行，禁止切换检查期间产生新业务动作。
- PositionProjection 保留对象唯一性、对象锁及来源 Transport/Evidence，只用于可靠更新和诊断。同箱再次合法使用由新的 WMS 业务决策
  和 Transport 成员事实决定；其他任务的旧投影或未知结果不阻断独立任务，真实同任务依赖仍等待匹配权威事实。
- 业务关闭不删除已保存的位置事实；后续动作发生后旧位置不能继续冒充当前权威位置。乱序或跨任务结果只应用于匹配任务，
  无法确认聚合当前位置时标记为未确认或未知，不据此建立本线占用门禁。
