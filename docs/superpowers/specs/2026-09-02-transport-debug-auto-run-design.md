...
现有“510056 现场联调步进”固定货架、料箱和路线，并依赖操作员逐步确认。现场已经打通 RCS → WMS → WES 链路，下一步需要把它收敛为一轮可恢复的自动诊断流程：操作员选择一个货架、按货架面分组选择该货架上的料箱并直接输入面值，后端根据 WMS 回调和 `SCAN12` Device Evidence 自动推进，所有选中料箱回架后才执行 `CTU03` 返库。

本设计的目标是：

1. 允许选择任意当前已挂载货架及其已挂载料箱。
...
- 同一料箱不能出现在多个组。
- 每个料箱当前均以 `MOUNTED` 状态挂载在所选货架上。
- 请求中的原始槽位与当前挂载关系完全一致。
- 当前没有占用 `GLOBAL` 的轮次。

校验通过后冻结配置。后续主数据变化不会偷偷改变执行目标；若执行时事实与冻结值冲突，则 fail-closed。
...
服务端从当前挂载事实校验并冻结完整配置，响应返回轮次详情。客户端不能覆盖固定站点、模板或返库面。
...
1. 从当前 `MOUNTED` 的 rack-bin mount 事实中选择货架。
2. 增加有顺序的货架面分组。
3. 每组直接输入非空面字符串。
4. 每组从该货架的当前挂载料箱中选择 1～4 个料箱；槽位随挂载关系展示并提交。
5. 禁止重复面字符串和重复料箱。
...
- 启动校验：挂载事实、槽位、空面、重复面、重复料箱、每组 1～4 个料箱。
