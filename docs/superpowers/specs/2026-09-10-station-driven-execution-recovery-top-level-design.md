...
状态：本次 WES 基础恢复与诊断范围已实现并验证：S1/S2、本地 `DEVICE_OBSERVATION` 留存、既有 WMS 原请求安全重试、后端历史/SSE 及 S5 前端展示。粗分插件机械传播已撤回；观察对外处理、WMS 响应纠正与粗分出口是后续独立业务需求，不是本次完成条件。当前实现保持供应商 ECS wire 不变；后端分支已推送，前端 PR 尚未合并，均未部署，也不代表现场验收通过。
...
WES 不因历史异常、原任务无响应或未完成业务对账，拒绝同一 ECS 交互点的新事件和后续正常请求；新请求可以形成独立上下文并可靠排队。ECS 负责现场执行互斥，WES 负责本次必要业务授权和消息一致性；旧动作未知时禁止重发该旧动作，并在取得权威终态前保留同设备物理派发围栏。

第一阶段只解决两条路径：

1. 排障后继续同一个 `task_id／command_code` 所指的原任务，正常结果到达后自动衔接。
2. 原任务异常或无响应，后续任务仍可进入同一 ECS 交互点并可靠排队；原未知动作取得权威终态后再派发后续命令，原任务作为历史异常保留，最终业务对账交 WMS。
...
本次只交付 WES 基础层的 Evidence/执行上下文、DeviceCommand 排队与重试、本地观察及诊断展示。不修改业务插件、SDK typed operation、WMS Adapter/Handler 或供应商合同。本文涉及 WMS 观察反馈、响应纠正及粗分出口决策的内容保留为未来设计背景，但不参与本次代码、测试、Review、发布或验收判定。
...
命令已超时或交付未知时，WES 保留原身份和未知事实，禁止盲目重发该动作，仍接收合同允许的迟到结果；新请求可以被接收并可靠排队，但同设备物理派发必须等待原未知动作取得权威终态。不能因本地已经告警就拒绝有效原结果，也不能把所有对账记录无差别重新执行。
...
纯自主输送段由 ECS/PLC 根据现场条件恢复。旧任务为确定 `FAILED` 时，后续有效业务请求按本次现场事实和 WMS 决定处理；旧任务为 `RECONCILING` 或无响应时，后续请求可以进入 WES 队列，但同设备物理派发必须等待原未知动作权威闭合。ECS 继续保证其所接纳动作的执行互斥，WES 不绕过 ECS 明确的拒绝或互锁。
...
下一个点位正常时，正常处理它自己的事件，不推定前一点位设备恢复。同一交互点新的有效业务请求可在旧任务异常或无响应时可靠入队；若原动作物理结果未知，则新命令等待其权威闭合后再派发。该点后续任务成功不能充当旧命令结果。已有未解除的显式设备故障按 ECS 状态处理，不因单个成功结果一并清空所有告警。
...
允许阻塞本次动作的条件限于本次业务授权缺失、本次身份或内容冲突、ECS 明确拒绝或互锁，以及明确的物理队列约束。旧任务 `FAILED` 不构成该 ECS 交互点的准入阻塞；旧任务 `RECONCILING` 或无响应不拒绝新请求入队，但在权威终态前继续冻结同设备物理派发。
...
| 异常与后续执行 | 原任务无结果时可接纳后续任务并排队；原未知动作权威闭合后才派发后续命令，后续成功不改写旧任务事实 | 旧结果缺失、新请求排队、权威闭合、后续成功及原结果迟到的完整时间线 |
...
Transport 保持自身确定结果、成员位置和资源围栏合同，本次实现未修改或推定放宽 Transport 行为；初始基线中的其他 Transport 现场不属于本计划范围。
...
| 原对象清除或旧任务无响应 | 新对象形成独立上下文并排队；原任务状态和事实保持不变，原未知动作权威闭合前不派发同设备后续命令 | 基础准入、插件与 WMS/ECS 联调分别验收 |
| 同点后续任务成功 | 原物理围栏已由权威事实闭合；旧 FAILED 保持失败，且不以新任务结果伪造旧终态 | 基础命令记录及展示消费者 |
...
- **T1（后续独立业务需求）**：闭合 D5 的 WMS 反馈合同；不属于本次实施或收尾条件。
- [x] **T2（P1，人类团队约 2–3 天 / Agent 约 4–8 小时）**：按 D1/D4 收敛基础准入、命令约束和原结果处理。范围为 `src/app/device/`、对应迁移及 `tests/runtime/device_command/`；验证同点独立新请求、精确归类、重复和乱序，闭合实际数据库/HEAVY 所有权。已由后端提交 `51533683` 闭合。
- [x] **T3（P1，人类团队约 1–2 天 / Agent 约 3–5 小时）**：按 D6/D7 统一派发与恢复。范围为现有派发、适配器及对应测试；验证 Status 忙碌有界重试、明确未接纳有界重试、未知不重发、崩溃恢复，复用现有 worker。已由后端提交 `51533683` 闭合。
- **T4（后续独立业务需求）**：闭合实际插件消费者、WMS 观察反馈与粗分出口决策；不属于本次实施或收尾条件。
- [x] **T5（当前范围）**：落实 D4 的当前/历史诊断展示。前端合同生成、聚焦测试、全量测试、构建和浏览器 QA 已由提交 `5f64da5` / PR #121 完成；不包含插件、WMS 对账或既有调试 API 退役。
...
关联、正式业务命令排队与调试安全槽分离。业务插件和新 WMS operation 已明确移出当前范围；后续授权已执行到后端 Push 与前端 Push/PR，尚未 Merge 或 Deploy。

已形成[本站驱动执行恢复实施计划](../plans/2026-09-10-station-driven-execution-recovery-implementation-plan.md)，当前按 S0、S1、S2、S3、S5、S6 闭合基础事件、命令、本地观察、界面与分层验证；S4 及对外业务合同另行立项。工程决定为：

- 按初始事件关联独立本站执行，取消未关闭物料 trace 作为跨点准入唯一锁；真实物理队列和 Transport 资源围栏不随之放宽。
- 本地未知/未接纳以独立 `DEVICE_OBSERVATION` 事实可靠持久化并用于诊断，不伪造 ECS 结果；本次不创建插件消费或新的 WMS 反馈义务。
- 明确 Status 动态忙碌及 `429/503` 可在原身份下有界重试，已 ACK 的接纳事实不回退；不新增供应商拒绝分类，不把不可信响应当作未接纳。
- 粗分出口独立决策及其 WMS 合同保留为后续业务设计，不进入当前实现。
- 同步数据库和诊断消费者；不修改 SDK typed operation 或正式插件。以已有测试承接新语义，真实 PostgreSQL/worker 与供应商现场验收分开记录。

首轮工程检查覆盖架构、代码质量、测试及性能，形成 D1–D8。后端主体提交 `51533683` 已实施 S1、S2、S3 本地观察/安全重试及 S5 后端诊断投影，最终生产代码快照 `cf2d783c` 补齐未知物理动作的设备派发围栏；前端提交 `5f64da5` / PR #121 已按主体后端提交完成 canonical contract freeze、诊断展示、测试及浏览器 QA。粗分插件机械传播已从当前范围撤回；R1 对外纠正、WMS 观察反馈和 S4 移为后续独立业务需求，不再使当前计划处于 PARTIAL。当前设计继续保留未来背景，硬件资料保持原文。

## GSTACK REVIEW REPORT

初始复评基线：`develop@06e56536`；当前后端可执行快照为 `cf2d783c`（主体能力提交 `51533683`），前端实现快照为 `5f64da5` / PR #121。D1–D8 保留，R1–R3 继续由 S0–S6 承接。当前状态以本报告和关联计划为准；文档检查不代替行为测试。
...
| Eng Review | `/plan-eng-review` + 固定 diff Review | 合同、事务、测试及切片 | 3 | CURRENT SCOPE COMPLETE | 内部基础切片与 S5 前端已实施并通过各自当前快照验证；业务插件和新 WMS 合同已移出当前范围 |
| Design Review | `/plan-design-review` | 当前状态与历史异常展示 | 0（本文） | NOT RUN | 未执行独立设计 Skill；前端聚焦测试与真实页面 QA 已完成 |
| DX Review | `/plan-devex-review` | 二次开发可读性 | 0（本文） | NOT RUN | 本轮已检查职责、复用及简单性，未执行独立 DX 评审 |

**VERDICT:** 本次 WES 基础与诊断范围已完成；业务插件、WMS 响应纠正、观察反馈和粗分出口不是当前 blocker。后端分支已推送、尚未合并，前端未合并，均未部署；不得以内部分支验证代替技术部署或供应商现场验收。

**后续独立业务需求（非当前 blocker）：**

- R1：具体 operation 的纠正/重新求值资格与旧义务权威闭合方式，按实施计划第 7 节在 S0 闭合。
- S3：`inbound.execution.observation_report@v1` 的严格请求、响应与 WMS 接收保证尚未联合批准。
