...
> 2026-09-11 目标补充：[WES 无阻塞执行与自动事实归集设计](2026-09-11-wes-nonblocking-execution-design.md)明确本架构中点位独立触发、异常阻塞范围和最终业务对账归属的收敛目标。对应具体合同尚待同步评审；本文件其余有效范围保留，不能把旧恢复条款或历史验收当作新目标已实现的证据。
...
1. WES 对每条 DeviceCommand 独立持久化和领取，不以本地 `AUTO + IDLE`、状态新鲜度或同设备活动命令作为发送前门禁；
   既有命令始终使用自身冻结合同，静态设备合同和业务参数仍须完整。
...
WES 不实现设备间或同设备物理互锁，也不使用状态投影授权发送；ECS 在接纳时判断设备状态、容量和实际互斥。
...
| ECS / PLC | 设备长命令执行、内部步骤、坐标、安全互锁、防撞、急停、复位、恢复执行和设备终态 | 急停安全事实留在 ECS；原 WES DeviceCommand 只通过统一结果/对账接口取得权威终态，供应商私有实现留在 ECS/网关边界 |
...
设备、安全或机械故障的物理判定与执行许可归 ECS。`ESTOP_PRESSED` 不是 WES callback 事件；WES 不保存、应用或据此自动改写原 DeviceCommand，只保留原身份和围栏，等待 ECS 以既有命令结果/对账合同报告权威终态。现场可以按设备或工作线配置更严格的停止策略；WES 不推测物理恢复，也不因 ECS 恢复而自动解除显式管理停用。
...
- COMMAND ACK 与最终 CALLBACK 明确分离；WES 可保存同设备多个独立未终态命令，ECS 负责原子接纳和物理互斥；每个命令最多一个已接纳终态结果。
...
- WES 不以单设备活动命令或本地 `AUTO + IDLE` 投影阻止新命令；ECS 在接纳时原子判断设备状态、容量和物理互斥。
- 目标位置满只阻止相关对象。
