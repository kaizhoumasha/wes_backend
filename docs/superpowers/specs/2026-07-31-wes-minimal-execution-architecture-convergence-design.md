...
- 测试以本文为基线：保留或改写最终业务行为断言，删除只验证旧架构、旧迁移和兼容路径的测试。
...
首次 CTU 单箱投料成功时绑定 `owner_workline_id` 和 `line_run_epoch_id`；没有可靠单箱投料回调时，由首次
...
- 位置、队列、货架、料箱和料格活动投影。
- 可观测性、安全和测试治理规则。
- 能直接证明本文目标行为的业务断言；测试代码按最终对象重写，不保留旧实现结构。
- 插件不能直接访问数据库或 HTTP 的边界。
...
1. 冻结本文定义的目标 ECS、WMS、RCS 合同、权威边界和可靠性不变量。
2. 建立生产代码、测试、迁移、配置和当前态文档的“保留、改写、删除”清单，并先建立禁止旧架构回流的门禁。
3. 建立最小执行对象、三类具体可靠投递记录和粗分机垂直切片。
4. 完成自动分拣线入库、人工分拣线、四线约束、复杂出库和满箱交换。
5. 删除旧 Runtime、System Capability、Manifest、Hold、Recovery、CellReservation 及其所有引用和测试。
6. 最终模型稳定后删除未发布迁移历史，使用 Alembic revision generator 创建单一干净基线。
7. 清空开发/测试数据库，从空库执行 `upgrade head`，再运行目标合同、领域、集成、重测试和质量门禁。
...
1. 当前态文档、TODO、逐文件删除清单和架构门禁。
2. 最小执行对象、具体可靠投递记录和粗分机垂直切片。
3. 自动分拣线入库、LIFO 投影和即时目标格计算。
4. 人工分拣线、四线 NG 透传和非 NG 同线进出。
5. 复杂出库来源执行和满箱交换。
6. 旧平台彻底删除、单一数据库基线、全量测试和最终收敛验收。

每个工作包分别形成实施计划和测试范围。后续工作包不得反向扩张最小执行内核；最终合并态不得包含
任何仅为旧版本、旧数据或迁移过程存在的代码。

### 14.4 测试与数据库基线

测试以本文目标行为为唯一基线：

- 当前测试若直接证明目标业务合同或可靠性不变量，改写到最终对象后保留。
...
| 按关键词删除测试误伤目标合同 | 逐文件语义判断（不建矩阵控制面），保留或改写最终行为断言，只删除旧实现断言 |
