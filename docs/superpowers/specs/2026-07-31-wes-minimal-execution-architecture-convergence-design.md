...
本文不是在现有 Runtime 上继续建设通用平台，而是以现有生产资产为基础进行架构收敛：

- 保留外部 HTTP 合同、幂等、持久化证据、数据库迁移、设备状态、资源投影和测试资产。
- 建立最小、具体、面向工作线对象推进的执行内核。
- 用代码插件承载客户和工作线差异。
- 按工作线逐条切换，再删除旧的通用 Runtime 平台。
...
Service Locator 和插件模板，不是简洁内核。实现必须从最新 `develop` 开始，保留其后形成的外部合同、
迁移和可靠性证据。
...
- HTTP/JSON EVENT、COMMAND、ACK、CALLBACK 外部合同。
- 入站先持久化再 ACK。
...
从最新 `develop` 创建收敛分支，保留外部合同和数据库迁移。旧提交只用于行为比对。

禁止：

- 将 `develop` reset 到历史提交。
- 以旧数据库模型重新生成迁移历史。
- 通过大规模树替换一次性重写所有工作线。
- 在同一 WorkLine 上长期双写两套运行时。

### 14.2 绞杀式迁移

迁移顺序：

1. 冻结外部 ECS、WMS、RCS 合同和现有行为测试。
2. 建立最小执行对象与粗分机垂直切片。
3. 迁移自动分拣线入库。
4. 迁移两条人工分拣线及四线 NG、同线进出约束。
5. 迁移复杂出库和满箱交换。
6. 清空旧 Runtime 引用并删除通用平台。

每个 WorkLine 只允许一个活动执行路径。切换时清空工作线并创建新 Epoch；旧路径只服务尚未迁移的其他工作线。

### 14.3 实施范围分解

本文是全局架构约束，不作为一个大 PR 的实施清单。实施必须拆成五个独立、可验收的工作包：

1. 最小执行对象、具体可靠投递记录和粗分机垂直切片。
2. 自动分拣线入库、LIFO 投影和即时目标格计算。
3. 人工分拣线、四线 NG 透传和非 NG 同线进出。
4. 复杂出库来源执行和满箱交换。
5. 旧 Runtime、System Capability、Manifest、Hold、Recovery 和 CellReservation 删除。

每个工作包分别形成实施计划、测试范围和切换门禁。后续工作包不得反向扩张最小执行内核。
...
| 风险 | 控制 |
| --- | --- |
| 删除通用 Runtime 时丢失可靠投递 | 先把可靠性落入三个具体执行记录，再逐 WorkLine 切换 |
