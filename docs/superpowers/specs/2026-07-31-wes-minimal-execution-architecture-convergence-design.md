...
ECS 每次物理搬运一个完整料盘。自动出库在设备扫码后由 WMS 唯一确定 `PkgID`，每个被接受的扫码结果对应一个
料盘级 `MaterialExecution`。SixInOne 中的 `Qty` 表示该包装内的物料数量，不用于展开多个料盘执行；物料、厂商和
批次等明细均来自 WMS 针对当前扫码返回的完整 6 合 1/PKG。

### 10.3 出库来源

`PkgID` 是完整料盘的唯一业务身份，不定义其他身份字段或兼容别名。`PickingTask` 冻结必须执行的 `BIN_CELL` 和
可选单盘 `RACK_SLOT`，不预绑定具体料盘；WMS 在任务启动时原子锁定 Cell 集合。设备从当前 Cell 顶部取盘并扫码后，
WMS 返回唯一 `PkgID`、完整 6 合 1/PKG、目标储位及 `CONTINUE | CELL_DONE`。WES 校验并建立作业期执行，不自行判断
物料资格、分配目标或决定 Cell 是否继续。
...
2. `PickingTask` 冻结必须执行的来源 Cell 集合，不预绑定 `PkgID`、SixInOne、目标转运货架或目标储位；WMS 在启动
   授权中原子锁定 Cell 并提供锁代际。
3. 目标空架、可选退料架和第一台五层货架允许并行调度；AGV 将指定五层货架送达 CTU 作业位并形成可靠证据后，
   CTU 才按冻结成员批次向入料缓存投箱。
4. CTU 投箱与当前料箱作业并行，只受入料缓存具体位置准入和 PLC/ECS 硬互锁背压；滚筒线按入料、工作位、退料三段
   逐位置投影运行，工作位容量包含机械臂实际工作位。
5. 料箱经 SCAN1 准入并到达实际工作位后，WES 选择该 Bin 的未完成 Cell；机械臂取盘到扫码台，WES 持久化扫码证据并
   请求 WMS 返回唯一 `PkgID`、SixInOne、业务接受/拒绝、当前开放目标面内的目标储位及 `CONTINUE | CELL_DONE`。
6. WES 不自行判断物料资格、目标储位或 Cell 是否继续。`CELL_DONE` 只停止下一盘；最后一盘 PUT/NG 物理闭合且无
   unknown 后，`CellExecution` 才完成。
7. 退料架流和 Bin/Cell 流可以并行准备，但共享机械臂、料盘扫码台和目标架工作面必须串行仲裁；目标面从 A 切到 B 后
   不得再返回 A 面储位。
8. 所有 `CellExecution` 完成即 `PickingTask` 完成；逐盘位置事实与任务完成事实分别可靠提交并保持因果顺序。
   Rack、Bin、Transport 和工作位清场由独立对象继续闭环，不参与 PickingTask 完成聚合；下一任务准入由
   当前 `LineRunEpoch` 下的设备/位置投影和活动对象共同判断，不新建通用 Session。
...
- 出库任务原子锁定 Cell 集合，设备从 Cell 顶部取盘并扫码后才由 WMS 唯一确定 `PkgID`、目标储位和是否继续。
- AGV 送达指定五层货架后 CTU 才能按冻结成员批次投箱；三段缓存按具体位置容量背压。
- 目标面单调推进；每个料盘位置事实与整单完成分别提交，未知物理结果阻止相关资源继续使用。
- 退货、转运货架单储位只能有一个完整料盘。
- 一个被 WMS 接受的扫码结果只关联一个唯一 `PkgID` 和一个完整料盘执行；`Qty` 只表示包装内物料数量。
- 所有 Cell 完成即 PickingTask 完成；Rack、Bin、Transport 和工作线清场不进入 PickingTask 完成聚合。
