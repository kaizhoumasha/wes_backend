...
> 注：下表所有"文件数"基线使用 `rg --files <path> | wc -l`（gitignore 过滤后的实际源码文件数），与 §Proposed Change 刷新命令保持一致。`find -type f` 因包含 `__pycache__` 等忽略目录，结果约为 2 倍，不可作为对照。
...
| `docs/architecture/workline-and-plugin-restructuring.md:2192` | Phase 0 启动时要求 `external-contract-profile-spec.md`、`integration-lab-and-simulator-spec.md`、`architecture-guardrails-spec.md` | 本 SPEC 将三者合并为 Phase 0 子章节,不单独分文件 |
...
| `src/app/workline/` | 89 个文件（`rg --files`） | legacy 清理矩阵必须逐入口分类 |
| `src/workline_runtime/` | 50 个文件（`rg --files`） | 旧 runtime 只能作为业务事实样本和 characterization 来源 |
| `src/workline_plugins/` | 12 个文件（`rg --files`） | 旧 plugin 框架不得作为目标态架构基础 |
| `tests/workline_runtime/` | 104 个文件（`rg --files`） | 行为契约测试要保护业务语义,不以旧代码覆盖率为目标 |
...
| 审计结论 | 本 SPEC 固化位置 |
| --- | --- |
| Phase 0 范围超过单个可 review PR，必须拆 3 个 PR 包 | §Proposed Change 的 PR1/PR2/PR3 拆分和 §Worktree Parallelization |
...
4. 无法分类的入口必须标记 `strategy=pending-review`，但 Phase 0 完成前 `pending-review` 数量必须为 0。
...
Phase 0 按 3 个 PR 包交付，完整范围不裁剪，但每个 PR 必须可独立 review：

| PR 包 | 范围 | 完成门禁 |
| --- | --- | --- |
| PR1: target + inventory | P0-001、P0-002a/b/c、P0-004、`guardrail_seed_scope` inventory | 目标态合同发布；legacy 和 seed 违规入口均有 `entry_id`；`session-correlation-matrix.md` 发布 |
| PR2: contracts + external boundary | P0-003、P0-005、P0-006 | BC-01/03/04/05/06 有目标态 contract test；BC-05/06 同时有 characterization 输入提取；DeviceCommand 与 external profile 不越界 |
| PR3: guardrails + quality gate | P0-007 | guardrails 脚本、allowlist、architecture tests、`git-quality-gate`/CI 接入完成 |

PR 划分约束：

1. PR1 必须先完成并合并；PR2、PR3 只能引用 PR1 稳定下来的目标态合同、legacy matrix `entry_id` 和 correlation matrix。
2. PR2 与 PR3 可在 PR1 后并行，但 PR3 必须消费稳定的 matrix ID 快照；若 PR2 仍在调整 `tests/support/` 或 `docs/architecture/` 公共合同，PR3 应顺序执行以避免 allowlist 反复漂移。
3. 每个 PR 的验收必须独立可 review，不允许把 “后续 PR 会补齐” 作为当前 PR 通过理由。
4. PR2 与 PR3 若并行，必须在 PR 描述中声明文件 owner：`tests/support/`、`docs/architecture/`、`tests/fixtures/` 只能由一个 PR 主改，另一个 PR 只引用已合并内容。
...
| `scripts/git-quality-gate.sh`、`Jenkinsfile` | PR3 接入 architecture guardrails 的现有质量门禁入口 | 只提供手动脚本、不接入本地门禁和 CI/Jenkins |
...
| legacy 入口无法分类 | 先写 `strategy=pending-review`，但 Phase 0 完成前必须归零 | workline 域 owner + architecture lead |
...
| 状态所有权 | WorkLine 配置、ExecutionSession/WorkItem、RuntimeIntentLog、RuntimeInbox、DeviceCommand、WMS facts 分别归属明确 |
...
| `drop_phase` | enum | yes | `phase2` / `phase3` / `phase4` / `phase5-tech` / `phase5-business` |
...
| BC-04 | `resource_projection/duplicate_active_owner.json` | 同一 object 同一 WorkLine 内第二个 active 归属失败，瞬态窗口有解释 | mock time source；不 mock uniqueness/policy 判断。瞬态窗口语义：指 RuntimeIntentLog effect commit 之间的 < N ms 间隔被视为合法重复（旧 owner 尚未释放、新 owner 已 claim 的物理时序），Phase 0 此字段占位 `transient_window_ms=PHASE1_PENDING`，具体 ms 数由 Phase 1 RuntimeIntentLog spec 给出 |
...
`ExecutionCorrelation` Phase 1 目标 schema 草案，供矩阵引用：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | bigint | yes | 内部主键 |
| `correlation_id` | string(120) | yes | 跨域稳定 correlation key，唯一 |
| `provider_code` | string(60) | yes | `WMS` / `ECS` / `WES` / provider-specific |
| `operation_kind` | string(80) | yes | query/effect/callback/work-item 类型 |
| `idempotency_key` | string(160) | nullable | 出站 effect 或入站事件幂等键 |
| `request_hash` | string(128) | nullable | 幂等 payload hash |
| `execution_session_id` | bigint | nullable | runtime/orchestration 内部引用 |
| `work_item_id` | bigint | nullable | 对象级执行令牌 |
| `external_ref_type` | string(80) | nullable | typed external reference type |
| `external_ref_value` | string(240) | nullable | 外部引用值 |
...
| 设备状态 | `IDLE/RUNNING/ERROR/OFFLINE` |
| 设备准入 | dispatch 前实时确认 ECS IDLE |
| RUNNING | 有界等待 |
| ERROR/OFFLINE/timeout | 短退避后 RuntimeHold |
...
| D4 | 当前阶段 RCS/AGV/CTU 调度只能经 WMS 履约 port；直连能力仅作条件触发扩展 |

脚本最小扫描规则：

| Rule | 命令形态 | 失败条件 |
| --- | --- | --- |
| C1 | `rg -n "from src\\.app\\.wms_integration\\.(services|models).* import" src/app --glob '!src/app/wms_integration/**'` | 内部域 import WMS implementation/DTO |
...
| C4 | `rg -n "(plc|coordinate|joint|axis|x_coord|y_coord|safety_loop)" src/app/device src/app/workline` | DeviceCommand/manifest 出现禁止字段 |
...
`scripts/architecture-guardrails.sh` 必须支持 `--phase` 参数，Phase 0 实施 PR 默认运行 `--phase phase0`，行为如下：
...
Phase 0 实施 PR 必须随脚本提交 `scripts/architecture-guardrails.allowlist` 的 **seed 版本**，预填以下当前已知违规，确保 Phase 1 切到 `enforced` 模式时不会因历史包袱直接 fail：
...
5. `scripts/git-quality-gate.sh` 增加 `--check architecture`；PR3 更新 CI/Jenkins 质量门禁调用，确保 Phase 0 默认跑 `--phase phase0`，Phase 1 起可切到 enforced 模式。
6. PR3 必须提交至少一条“临时删除 seed 行触发失败”的本地验证记录到 PR 描述，避免脚本只验证 happy path。

## Acceptance Criteria

1. `target-state-contract.md` 发布，不含旧 API、旧表或旧 plugin 兼容承诺。
...
[+] PR1 target + inventory                          [+] Architecture reviewer validates scope
  ├── [planned] 3 PR package split                    ├── [planned] total_entries_by_type check
  ├── [planned] legacy matrix counts                  └── [planned] guardrail_seed_scope traceability
  └── [planned] session correlation matrix

[+] PR2 behavior contracts                          [+] Runtime behavior remains protected
...
[+] PR3 guardrails                                  [+] Maintainer runs quality gate
...
| **P0-002a** legacy matrix: workline + workline_runtime | L | 1.5-2 天 | rg --files 共 139 个文件，预估 300+ entry |
| **P0-002b** legacy matrix: workline_plugins + tests | M | 0.5-1 天 | rg --files 共 116 个文件，预估 130+ entry |
...
总计：CC + gstack 约 **6.5-9 天**（P0-002 拆 sub-package 后下限提高，避免单 PR review 失败重做）。人工团队约 **3-4 周**，取决于 review、旧业务语义确认和逐入口 inventory 数量。

## Implementation Tasks

这些任务来自 eng review 后的执行收敛清单，以及本轮文档边界收敛新增的 Phase 0 PR 复查项。实现阶段按 PR 包拆分，逐项勾选，不再把审计附录当作执行依据。

- [ ] **T1 (P1, human: ~1h / CC: ~15min)** — Phase 0 交付拆分 — 在实施 issue 中保留 3 PR 包拆分。
  - 来源：scope review 发现 Phase 0 范围超过一个可 review PR。
  - 文件：`docs/superpowers/specs/2026-06-25-workline-restructuring-phase-0-spec.md`
  - 验证：Issue/PR 列表与 PR1/PR2/PR3 依赖表一致。
...
| --- | --- | --- |
| PR1 target + inventory | `docs/architecture/`、`src/app/workline/`、`src/workline_runtime/`、`src/workline_plugins/`、`tests/workline_runtime/`、seed-scope source dirs | none |
| PR2 contracts + external boundary | `docs/architecture/`、`docs/contracts/`、`tests/contracts/`、`tests/characterization/`、`tests/fixtures/`、`tests/support/` | PR1 target contract and matrix IDs |
| PR3 guardrails + quality gate | `scripts/`、`tests/architecture/`、`tests/support/`、`Jenkinsfile`、`docs/architecture/` | PR1 matrix IDs and PR2 contract boundaries |

执行建议：

1. 先单独实现并合并 PR1。
2. PR1 合并后，PR2 与 PR3 可并行 worktree 执行；若 PR2 仍在调整 `tests/support/` 或 `docs/architecture/` 公共合同，PR3 应等待 PR2 稳定后再接 allowlist。
3. PR2 与 PR3 都会触碰 `tests/support/` 和 `docs/architecture/`，并行时必须约定文件 owner；否则顺序执行更稳。
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 fresh | CLEAR | 7 issues, 0 critical gaps, 0 unresolved |
...
- **VERDICT:** ENG CLEARED — ready to implement Phase 0 as PR1/PR2/PR3.
