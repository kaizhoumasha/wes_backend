...
- [x] **T5（P1，human: \~5d / CC: \~1d）** — EFFECT — 接入 9 项同步 EFFECT、7 项异步 EFFECT 与双 WMS lane
  ...
  - Verified checkpoint — G4.5b1 / E11 runtime domain authority：唯一 System Capability pipeline
    ...
    静态 allowlist 只允许 `SMT_INBOUND_HANDOFF → wms.fulfillment.full_box_exchange@v1`。production
    resolver 以 `FOR UPDATE` 锁定 `ExecutionCorrelation → SmtInboundHandoffDemand → WorkLine`，并从
    持久化 owner/release/workline 事实派生 producer；caller ORM-like 对象不参与授权。domain claim 的
    session/work-item/plugin/binding identity 均为空，correlation 保持既有 FK，producer、business owner、
    workline 与 correlation anchor 冻结进入 `binding_snapshot_json`，幂等键固定为
  ...
  - G4.5b1 evidence：authority/model/migration 与既有 plugin coordinator 定向覆盖 `71 passed`，真实
    PostgreSQL cold-start effect 文件 `5 passed`，覆盖 nullable FK、三张权限事实行锁、首次 claim、
