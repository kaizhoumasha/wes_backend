...
> **Completion status (2026-07-07):** DONE for material-flow runtime readiness. development/mock readiness、sorter inbound 与 SMT/NG/WMS reconciliation production-capable runtime path、evidence profile gate 与 composer 已闭合；material-flow production evidence artifact 已由 `2026-07-07-phase3-production-artifacts.md` 生成并通过 production gate。Phase5 business cleanup 已随 PR #79 合并；真实生产发布仍需现场 evidence/canary 按发布流程执行。
...
sorter inbound 与 SMT/NG/WMS reconciliation 后续目标是 production-capable runtime path，外部 provider 可替换。WES 代码只面向 provider contract、RuntimeIntent、RuntimeInbox、idempotency、timeout/retry、evidence 与 RuntimeHold/Reconciliation；外部是真设备、sandbox、MOCK 还是 simulator 由部署 wiring 与 evidence profile 区分，不进入业务代码分支。Phase1 callback admission 已关闭；本项目未发布，当前开发/测试默认使用 MOCK closure，真实 artifact 不再作为当前开发/测试推进阻塞项。发布前仍必须显式通过 `scripts/check_runtime_production_closure_gate.py --closure-profile production ...` 与 material-flow evidence profile gate。
...
2026-07-05 追加进展：新增 `Phase4SorterInboundRuntimeService` 与 `SmtNgWmsReconciliationRuntimeService`，把 sorter inbound 与 SMT/NG/WMS reconciliation 从 preview 语义推进到 production-capable runtime path 的 plan builder：输出 `RuntimeIntent`、effect port contract、provider-contract evidence、RuntimeInbox evidence 与 RuntimeHold scope-only plan，不判断外部 provider 是真设备、sandbox、MOCK 还是 simulator。`scripts/check_runtime_evidence_readiness_gate.py` 新增 `development/simulator/site/production` evidence profile；profile 只改变证据要求，不改变 service 行为。新增 `scripts/compose_runtime_evidence_artifact.py`，用于生成可被 gate 验收的 material-flow runtime evidence artifact，避免手写证据口径漂移。

2026-07-05 追加进展 2：material-flow runtime evidence profile gate 已细分 `simulator/site/production` 证据要求。`simulator` 允许最小 provider-contract artifact；`site/production` 必须提供 `evidence_manifest`，引用 sorter inbound / SMT-NG-WMS reconciliation provider contract、effect dispatch trace、RuntimeInbox worker trace、RuntimeHold/ReconciliationRecord trace 与 material-flow runtime benchmark evidence 文件。`production` profile 继续叠加 Phase3 production closure artifact 校验。composer 支持 `--evidence-dir` 组装 site/production artifact；证据文件本身属于 `reports/` 或 CI/deploy artifact，不提交到 git。
...
- material-flow preview 与 mock 合同只作为语义基线；runtime capability builder 必须只面向 provider contract，不根据外部 provider 是否 MOCK / simulator / 真设备选择业务分支。
- evidence profile 只改变验收证据要求；`site/production` 缺 material-flow evidence manifest、引用 evidence 文件、Phase3 production closure artifact 或 benchmark artifact 时，发布前 gate 保持红灯。
...
- `uv run python scripts/compose_runtime_evidence_artifact.py --output reports/phase4/runtime-evidence-simulator.json --profile simulator --environment local-wms-ecs-simulator --generated-at 2026-07-05T00:00:00Z`
- `uv run python scripts/compose_runtime_evidence_artifact.py --output reports/phase4/runtime-evidence-site.json --profile site --environment field-dry-run --generated-at 2026-07-05T00:00:00Z --evidence-dir evidence/phase4-runtime`
- `uv run python scripts/check_runtime_evidence_readiness_gate.py`
- `uv run python scripts/check_runtime_evidence_readiness_gate.py --readiness-profile simulator --runtime-evidence-artifact reports/phase4/runtime-evidence-simulator.json`
- `uv run python scripts/check_runtime_evidence_readiness_gate.py --readiness-profile site --runtime-evidence-artifact reports/phase4/runtime-evidence-site.json`
