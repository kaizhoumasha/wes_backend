...
**Status:** In progress — Phase 1–7 已完成，Phase 8 后端 RC 已关闭，Phase 9 已合入 `develop@c5a93872`。Phase 10 Tasks 0–6 已在
`codex/phase10-implementation@834fe59e` 按 Task 独立提交并完成仓内工程/准入验证；Task 7 Deploy/Cutover 未执行，分支尚未
Push、创建 PR 或合入 `develop`。Phase 8 的 GitLab 发布提交
`f51677b62f5da906d4b60fa5a528d04692aff7a2` 已由 Jenkins #88 生成不可变后端 RC 镜像 `88-f51677b`。
真实 WMS/RCS/ECS、设备、版本组合、现场部署和业务验收不属于上述仓内完成证据。
...
**Phase 10 candidate baseline:** `codex/phase10-implementation@834fe59e0c44c943487eedb6ed41af1c519df7ad`
（source tree `58fbe212ba57186668783eb06f85c7cf37a0d7a6`，未合入、未部署）；
...
合入 backend `develop@c5a93872`；Phase 10 仓内候选尚未合入或执行 Task 7。Phase 8 RC 发布证据仍为
...
本十四阶段计划只交付顶层 SPEC §3.1 已确认的粗分机、人工 Bin、自动上架和自动拣货能力。
SRS §3.5 特殊物料、机构件/SFC 协同及 §3.6 生产退料属于未来需求，不进入 Phase 8/12/13 或 Phase 14 验收，
...
| 文档或引用 | 影响 | 本轮处理 |
| --- | --- | --- |
| `docs/superpowers/plans/2026-08-03-wes-architecture-convergence-master-plan.md` | 总控重排为十四阶段 | 本轮同步 Phase 9–14 |
...
| `docs/architecture/file_index.md` | 当前索引仍称旧阶段数 | 同步为十四阶段并新增当前计划入口 |
| `README.md` / `docs/architecture/SRS.md` | 若仍写旧阶段数则编号过期 | 本轮扫描并机械同步；业务范围不变 |
...
| `34837439` / `src/app/runtime/workline_plugins/` 缺席 | 旧工作线插件、binding、dispatcher、attempt 及目录外活动 owner 已原子退役 | Phase 5 零插件基线完成；后续插件按 Phase 8/12/13 目标合同重新实现 |
...
Phase 10 Tasks 0–6 已在 `codex/phase10-implementation@834fe59e` 完成仓内准入，Task 7 未部署；Phase 11–14 尚未开始。
...
**Objective:** 在 Phase 2 已交付唯一新基础层、Phase 3–8 和 Phase 9 最小基础替代均已完成的基础上清除跨阶段残留，证明生产态只有
最小执行架构和唯一 HTTP 基础层。
...
**与前后阶段的 atomic handoff:** 接收 Phase 3–8 与 Phase 9 Foundation 的删除余量并核验 Phase 2 新基础层的唯一性；全部零命中后才允许
...
`codex/phase10-implementation@834fe59e` 完成；Task 6 复用并核验既有发布静默基线，未重复建设。Task 7 必须另行取得
Deploy/Cutover 授权并以同一不可变候选执行；在此之前不得把 Phase 10 标为完成，也不得启动 Phase 11。
...
**Entry conditions:** Phase 10 退出门禁通过；Phase 9 已批准且已实现的最小执行内核、当前 WMS/RCS Adapter、设备统一接口和
`rough_sorter` 所需持久化模型稳定；无旧表活动消费者。
...
**Objective:** 按真实合同分别交付 `automatic_putaway` 与 `automatic_picking`，复用 Phase 12 已验证的插件边界，不建设通用工作流。

**Entry conditions:** Phase 12 教学式切片完成；相应 WMS/设备合同、真实拓扑和部署组合已批准。

**Exit gate:** 每个插件独立拥有业务测试、迁移、Composition、制品和验收；核心无插件 import、fixture 或供应商私有分支。

## 19. Phase 14：当前交付范围系统验收

**Objective:** 从干净环境证明最小执行内核、Phase 2 HTTP、当前 Adapter、设备统一接口、实际交付插件、数据库基线、部署装配和缺席门禁。

**Entry conditions:** Phase 13 当前范围插件完成；所有阶段代码、合同和当前态文档一致。

**Scope:** 空库 upgrade、分层 FAST/QUALITY/HEAVY、Adapter/设备/插件独立验收、部署与 Composition、旧架构和重复 HTTP 缺席门禁。

**Exit gate:** 当前范围全部验收通过；未执行的供应商一致性、现场联调或物理动作明确标记为未验收；不得在最终验收阶段临时实现缺失能力。

## 20. 发现的矛盾与最终裁决

| 矛盾或歧义 | 仓库证据 | 最终裁决 |
| --- | --- | --- |
| 旧总控把 WMS 作为 Phase 2 | Master/WMS 子计划旧编号 | Phase 2 独立为公共 HTTP；WMS 为 Phase 3，最终验收顺延到 Phase 14 |
...
| 统一设备 HTTP Adapter 曾被塞入 Phase 4 或插件阶段 | 旧摘要把 DeviceCommand、设备状态和 ECS HTTP 与 Transport/粗分混建 | Phase 7 成为正式独立 Device/ECS 阶段；Phase 8/9/12/13 只消费，不修补核心能力 |
...
| SPEC §14 仍是旧阶段顺序 | SPEC §14.2–14.3 | 同步为十四阶段：Phase 9 最小基础、Phase 10/11 清理、Phase 12/13 插件、Phase 14 验收 |
...
| 重复职责 | 通过 | Phase 5 只退役旧插件闭包；Phase 6 只拥有 Transport；Phase 7 只拥有 Device/ECS；Phase 9 只拥有最小基础；Phase 12/13 各自拥有插件 |
| 测试过重 | 通过 | Phase 5 按 owner 处置旧测试；Phase 6/7/9 分别验证基础能力；Phase 12/13 插件测试不进入核心 |
...
| 当前状态准确性 | 通过 | Phase 1–7 已完成，Phase 8 后端 RC 已关闭，Phase 9 已合入；Phase 10 Tasks 0–6 已在 `834fe59e` 完成仓内准入，Task 7 未部署；Phase 11–14 未开始；Phase 8 外部验收边界仍以 `docs/integration/rough-sorter-joint-acceptance.md` 为准 |
...
| 6 | Phase 10 Tasks 0–6 | Implemented and verified | 七个 Task 提交、QUALITY `2378 passed`、staged HEAVY `439 passed`、fresh Review 0 findings、本地候选健康；未 Push/PR/Merge/Deploy |
| 7 | Phase 10 Task 7 原子 cutover | Not run | 需要单独 Deploy/Cutover 授权；现场 drain、连续 READY、旧 worker 停止和 Phase 11 handoff 尚无证据 |

Phase 8 后端实施、本机 Mock 和不可变 RC 镜像发布已完成。这不表示供应商一致性、真实 RCS 顺序能力或现场业务闭环已通过。

## 24. 工程复审完成摘要

- **Scope：** Phase 5 只退役旧插件执行闭包；Phase 6 只收敛 Transport；Phase 7 只收敛 Device/ECS；Phase 9 只闭合最小基础；Phase 12/13 才交付目标业务插件。
...
- **Parallelization：** Phase 8 后端 RC 关闭后可立即推进 Phase 9 后端开发；前端 RC 和现场活动独立推进。现场差异按 owner 修复，
  不并行改造核心公共 wire。
- **NOT in scope：** Phase 7/9 不实现 Phase 12/13 的业务插件；任何阶段都不得删除 `docs/hardware/`。
...
| SEQUENTIAL REVIEW | CLEAR | 1 | 0 | 十四阶段顺序通过依赖、YAGNI 和测试所有权复核 |
...
**VERDICT：十四阶段职责边界保持冻结；Phase 1 至 7 核心基线均已完成；Phase 8 后端 RC 已关闭并发布不可变镜像
