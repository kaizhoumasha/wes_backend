...
`PASS/GO` 表示 `docker-compose.wms-acceptance.yml` 启动的已构建 `mock_wms_acceptance` 镜像已通过全部已冻结 typed EFFECT
的真实 TCP 黑盒合同验收，当前开发
...
| EFFECT status query | 按 `operation_identity + idempotency_key` 返回五态、单调 `source_version`、稳定拒绝码和 operation-specific typed result | 对全部已冻结 EFFECT 逐项回放可见状态、终态、`NOT_FOUND`、429、5xx、超时和响应体上限 |
...
恰好由一个匹配请求消费。全部已冻结 EFFECT 的 `COMPLETED` 必须分别通过对应 typed result 校验。
