...
`transport_submit_path: /api/v1/wes/transport-requests`。该 Provider Mock 从同一 profile 注册29项公开路由，并复用现有 typed
operation fixture；`check` 会实际调用代表性的 query 与 effect。8011端口的 Transport Mock保持独立，不承载WMS业务Provider接口。
