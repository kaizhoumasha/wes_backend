...
`WMS_EFFECT_ADMISSION_ENABLED` 是唯一 EFFECT 准入开关，只允许一次性启停静态 registry 中全部 16 项 EFFECT。
operation identity 仅用于观测、诊断和定位，不存在按 operation 局部暂停、灰度开放或逐项恢复。
...
| --- | --- | --- | --- |
| `wms_effect.submit` 非 `ACCEPTED` | 5 分钟窗口触发现有 operation SLO `14.4` fast burn，或 p95 超过 catalog 对应阈值 | 将 `WMS_EFFECT_ADMISSION_ENABLED=false`，一次性暂停全部 16 项新 EFFECT；保留 status/reconciliation worker | 连续一个 SLO 快窗口低于阈值，且全部既有 accepted/ambiguous 均可由 status/reconciliation 解释 |
| `wms_effect.status_backlog.max_overdue_age_ms` | 连续两个扫描周期大于 `2 × WES_EFFECT_STATUS_SCAN_PERIOD_SECONDS × 1000` 为 warning；持续达到 `WES_EFFECT_STATUS_SCAN_BATCH_BUDGET_SECONDS × 1000` 为 stop-admission | 先检查 scanner/lease，再将全量 EFFECT 准入开关置为 `false`；禁止通过扩大并发绕过 WMS 限流 | 连续三个扫描周期低于 `WES_EFFECT_STATUS_SCAN_PERIOD_SECONDS × 1000`，且无超预算 intent |
| `wms_effect.status_backlog.max_confirmation_age_ms` | 达到 `WES_EFFECT_MAX_CONFIRMATION_AGE_SECONDS × 800` 为 warning；达到 `WES_EFFECT_MAX_CONFIRMATION_AGE_SECONDS × 1000` 为 stop-admission | 将全量 EFFECT 准入开关置为 `false`，保留 status/reconciliation worker，检查 WMS 确认链路 | 连续三个扫描周期低于 warning 阈值，且无超预算 intent 或未关闭 reconciliation |
| `wms_effect.status_backpressure` | 任一 `RATE_LIMITED`/`CIRCUIT_OPEN` 进入诊断；连续三个扫描周期仍出现或 backlog 达 stop-admission 阈值则暂停准入 | 将全量 EFFECT 准入开关置为 `false`；遵守 `Retry-After` 和 actual backoff，保留 status worker，不手工抢跑 | 连续三个扫描周期无 `RATE_LIMITED`/`CIRCUIT_OPEN`，且 backlog age 持续下降 |
...
1. 确认告警对应的唯一 provider profile 与 operation，按 operation 记录当前 backlog、oldest age、429、实际退避和
   UNKNOWN；该维度只用于诊断。
2. 设置 `WMS_EFFECT_ADMISSION_ENABLED=false`，一次性停止全部 16 项新 EFFECT；不得局部启停、构造第二
   Provider 做运行时 fallback 或按 payload 动态路由。
3. 等待 active lease 归零；有过期 lease 时先执行 fencing/对账流程，禁止直接重放 UNKNOWN。
4. 全部恢复判据满足后一次性设置 `WMS_EFFECT_ADMISSION_ENABLED=true`，恢复全部 16 项；恢复后按 operation
   观察一个 SLO 快窗口，但不得把观测顺序实现为局部开放。
5. 在审计中记录责任人、catalog/policy version、全量暂停和全量恢复时间。
...
4. backlog age 持续增长时将 `WMS_EFFECT_ADMISSION_ENABLED=false`，一次性关闭全部 16 项新 EFFECT，并保留
   status/callback/reconciliation worker；不要增加并发绕过 WMS 已给出的限流事实。
...
4. 通过脱敏凭据审计确认 `RESOLVED`，再按暂停/恢复流程一次性恢复全部 16 项 EFFECT 准入。
...
4. lease loss 持续增长时将唯一 profile 的 `WMS_EFFECT_ADMISSION_ENABLED=false`，排查 worker 时钟、数据库锁和
   网络。
