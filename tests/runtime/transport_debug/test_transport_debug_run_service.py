...
async def test_abort_requires_attention_and_terminal_tasks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    audit = _Audit()
    monkeypatch.setattr("src.app.transport_debug.debug_run_service.audit_log_service", audit)
...
async def test_abort_finalizes_only_a_provably_unsent_debug_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.app.transport_debug.debug_run_service.audit_log_service", _Audit())
