...
def test_runtime_contract_guardrails_check_is_available(tmp_path):
    """runtime contract guardrails 必须可作为 standalone check 执行。"""
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir(parents=True, exist_ok=True)
    fake_uv = fake_bin / "uv"
    fake_uv.write_text(
...
""",
    ...
    result = subprocess.run(
        ["/bin/bash", str(QUALITY_GATE), "--check", "runtime-contract-guardrails"],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
