...
@pytest.mark.asyncio
async def test_generated_effect_exception_rolls_back_once_without_terminal_write() -> None:
    ...
    class IntentRepository:
        def prepare_attempt_intents(self, **_kwargs: object) -> tuple[object, ...]:
            return (SimpleNamespace(claim={"idempotency_key": "intent-1"}),)
    ...
    class FailingEffectApplier:
        async def apply(self, ctx: dict[str, Any], _intents: list[object]) -> object:
            assert "orch_result" not in ctx
            assert isinstance(ctx["effect_state"], _effect_state_type())
    ...
    with pytest.raises(RuntimeError, match="effect failed"):
        await RuntimeInboxWriteBackService(
            ...
            effect_applier=FailingEffectApplier(),
            ...
            write_set=AttemptWriteSet(evidence=(), next_state={"phase": "NEXT"}, intents=("intent",)),
        )

    assert events == ["intent", "effect", "rollback"]
