...
def _intent(value: str = "A") -> RuntimeIntent:
    return RuntimeIntent.system_capability(
        ...
        authorization_policy="policy-v1",
...
def _ctx(db: _Db | None = None) -> dict[str, object]:
    return {
        ...
        "inbox": SimpleNamespace(correlation_id="corr-1"),
        "trace_id": "trace-1",
    }


def _service(definition: SystemCapabilityDefinition, guard: _Guard) -> SystemCapabilityEffectService:
...
async def test_local_transactional_effect_flushes_without_owning_commit_or_rollback() -> None:
    ...
    db = _Db()
    result = await _service(definition, _Guard(ClaimResult.NEW)).apply(_ctx(db), _intent())
...
async def test_outbox_async_success_means_durably_accepted_not_remote_completed() -> None:
    ...
    result = await _service(definition, _Guard(ClaimResult.NEW)).apply(_ctx(), _intent())
...
async def test_same_final_key_and_hash_is_noop_success() -> None:
    _RecordingHandler.calls.clear()
    guard = _Guard(ClaimResult.MATCH)
    result = await _service(_definition(), guard).apply(_ctx(), _intent())
...
async def test_same_final_key_with_different_hash_fails_closed() -> None:
    ...
    result = await _service(_definition(), _Guard(conflict)).apply(_ctx(), _intent("B"))
...
async def test_stale_precondition_remains_business_reject_for_plugin_redecision() -> None:
    result = await _service(_definition(_StaleHandler), _Guard(ClaimResult.NEW)).apply(_ctx(), _intent())
...
async def test_unknown_handler_exception_is_redacted_retryable_unknown() -> None:
    result = await _service(_definition(_FailingHandler), _Guard(ClaimResult.NEW)).apply(_ctx(), _intent())
