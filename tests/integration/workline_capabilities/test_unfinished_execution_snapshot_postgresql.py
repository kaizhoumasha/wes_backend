...
async def _seed_picking_snapshot(db, *, now: datetime):  # type: ignore[no-untyped-def]
    ...
    await db.flush()
    epoch = LineRunEpoch(
        epoch_code=f"PICKING-SNAPSHOT-EPOCH-{identity}",
        workline_id=line.id,
        plugin_key="snapshot_test",
        plugin_version="1.0",
        flow_mode="GENERIC",
    ...
    db.add(task)
    await db.flush()
    return line, epoch, task, evidence
...
async def test_picking_snapshot_counts_bound_active_and_blocked_tasks_only(integration_session_factory) -> None:
    ...
    async with integration_session_factory() as db:
        transaction = await db.begin()
        try:
            line, _, task, evidence = await _seed_picking_snapshot(db, now=now)
            other_line, _, other_task, _ = await _seed_picking_snapshot(db, now=now)
...
async def test_picking_confirmation_snapshot_needs_no_material_owner_and_counts_each_identity_once(
    ...
    async with integration_session_factory() as db:
        transaction = await db.begin()
        try:
            line, _, task, _ = await _seed_picking_snapshot(db, now=now)
            _, _, other_task, _ = await _seed_picking_snapshot(db, now=now)
            confirmation = WmsConfirmation(
