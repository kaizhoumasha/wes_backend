...
SCENARIOS = (
    ...
    Scenario("inbound-global-applied", "inbound", "APPLIED", None, "READY", "global"),
    Scenario("inbound-global-pending", "inbound", "PENDING", "inbound_evidence_wait_drain", "WAIT_DRAIN", "global"),
    Scenario("inbound-global-reconciling", "inbound", "RECONCILING", "inbound_evidence_block", "BLOCK", "global"),
    Scenario("inbound-workline-wms-applied", "inbound", "APPLIED", "inbound_evidence_wait_drain", "WAIT_DRAIN"),
...
async def _seed_scenario(db: AsyncSession, scenario: Scenario) -> None:
    ...
    if scenario.ledger == "device":
        ...
        row = InboundEvidence(
            ...
            normalized_payload={},
            received_at=now,
            workline_id=None if scenario.shape == "global" else workline.id,
