...
async def test_debug_reset_deletes_only_selected_postgresql_aggregate(
    ...
    try:
        ...
        assert (result.deleted_member_count, result.deleted_binding_count) == (1, 1)
...
async def test_debug_reset_preserves_postgresql_task_with_evidence_or_outcome(
    integration_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    suffix = uuid.uuid4().hex
    ...
    try:
        ...
        assert preview.blockers == ("TRANSPORT_EVIDENCE_EXISTS", "TRANSPORT_OUTCOME_EXISTS")
        with pytest.raises(TransportContractError, match="TRANSPORT_EVIDENCE_EXISTS,TRANSPORT_OUTCOME_EXISTS"):
            await service.reset_debug_task(task_id)
