...
def _reset_service(
    ...
    return TransportDebugResetService(sessions, repository or TransportRepository(), TransportDebugRunRepository())
...
async def test_debug_reset_deletes_only_selected_postgresql_aggregate(
    ...
    try:
        preview = await _reset_service(integration_session_factory).preview_debug_task_reset(target_id)
        assert preview.member_count == 1

        result = await _reset_service(integration_session_factory).reset_debug_task(target_id)
...
async def test_debug_reset_deletes_diagnostic_aggregate_but_preserves_core_projection(
    ...
    try:
        preview = await _reset_service(integration_session_factory).preview_debug_task_reset(task_id)
        ...
        result = await _reset_service(integration_session_factory).reset_debug_task(task_id)
...
async def test_debug_reset_does_not_delete_another_task_projection_when_operation_id_is_reused(
    ...
    try:
        await _reset_service(integration_session_factory).reset_debug_task(target_id)
...
async def test_callback_waiting_on_debug_reset_lock_is_retained_as_missing_task_conflict(
    ...
    reset_service = _reset_service(integration_session_factory, reset_repository)
...
async def test_debug_reset_rejects_task_referenced_by_active_debug_run(
    ...
    try:
        with pytest.raises(TransportContractError, match="active transport debug run task cannot be reset"):
            await _reset_service(integration_session_factory).reset_debug_task(task_id)
