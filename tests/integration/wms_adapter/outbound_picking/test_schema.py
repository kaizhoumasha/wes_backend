...
async def test_picking_task_issued_migration_builds_the_reviewed_postgresql_schema() -> None:
    async with temporary_database() as (_database, database_url):
        ...
        try:
            ...
            epoch_owner_column = await connection.fetchrow(
                """
                SELECT data_type, is_nullable
                FROM information_schema.columns
                WHERE table_schema = 'wes_biz' AND table_name = 'wms_confirmations'
                  AND column_name = 'line_run_epoch_id'
                """
            )
    ...
    assert tuple(epoch_owner_column) == ("bigint", "YES")
    assert confirmation_constraints["fk_wms_confirmations_line_run_epoch_id_line_run_epochs"] == (
        "FOREIGN KEY (line_run_epoch_id) REFERENCES wes_biz.line_run_epochs(id)"
    )
    owner_check = confirmation_constraints["ck_wms_confirmations_wms_confirmation_exactly_one_owner"]
    for owner in ("material_execution_id", "bin_execution_id", "picking_task_id", "line_run_epoch_id"):
        assert f"{owner} IS NOT NULL" in owner_check
    assert "= 1" in owner_check
    assert "ix_wes_biz_wms_confirmations_line_run_epoch_id" in confirmation_indexes
