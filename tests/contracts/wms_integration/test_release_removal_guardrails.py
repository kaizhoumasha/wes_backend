...
REMOVED_ACTIVE_DOC_LITERALS = frozenset(
    {
        ...
        "POST /api/wms/inventory/reserve\n",
        "DELETE /api/wms/inventory/reserve",
        "`POST /api/wms/inventory/transfer`",
        "POST /api/wms/inventory/transfer\n",
        "/api/wms/inventory/putaway",
        "/api/wms/inventory/issue",
        "/api/wms/fulfillment/move-bins-from-conveyor-exit",
        "TTL ≤ 30秒",
        "TTL 不得超过 30 秒",
        "短暂快照缓存（TTL 30s）",
        "30s TTL 缓存",
...
def test_callback_ingress_reuses_callback_domain_wms_allow_set() -> None:
    ...
    assert any(
        ...
        and node.module == "src.app.callback.contracts.external_callbacks"
        and any(alias.name == "WMS_ALLOWED_CALLBACK_TYPES" for alias in node.names)
