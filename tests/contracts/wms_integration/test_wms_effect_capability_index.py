"""16 项 WMS EFFECT 的 System Capability 静态索引合同。"""
...
def test_all_effect_operations_are_thin_shared_capability_compositions() -> None:
    assert len(EFFECT_OPERATIONS) == 16
...
def test_effect_mode_and_lane_are_the_exact_static_nine_seven_matrix() -> None:
    sync_identities = {
        operation.identity
        for operation in EFFECT_OPERATIONS
    ...
    assert len(async_identities) == 7
    assert len(data_identities) == 8
    assert len(fulfillment_identities) == 8
