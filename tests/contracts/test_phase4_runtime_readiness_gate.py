...
def _write_phase3_closure_artifacts(tmp_path: Path) -> tuple[Path, Path]:
    ...
    return (
        ...
        _write_json(tmp_path / "phase3-production-benchmark.json", benchmark_artifact),
    )
...
def test_phase4_runtime_readiness_gate_site_profile_requires_evidence_manifest(tmp_path) -> None:
    evidence_artifact = _write_json(
        tmp_path / "phase4-runtime-evidence-site.json",
        _phase4_runtime_evidence_artifact(profile="site"),
    )
    ...
    assert "MISSING_PHASE4_RUNTIME_EVIDENCE_MANIFEST" in result.stdout


def test_phase4_runtime_readiness_gate_site_profile_accepts_evidence_manifest_files(tmp_path) -> None:
...
def test_phase4_runtime_readiness_gate_production_profile_requires_phase3_artifacts_after_phase4_evidence(
    ...
    assert result.returncode == 2
    assert "MISSING_PHASE3_CLOSURE_ARTIFACTS" in result.stdout


def test_phase4_runtime_readiness_gate_production_profile_accepts_phase4_and_phase3_evidence(tmp_path) -> None:
    evidence_dir = tmp_path / "evidence" / "phase4-runtime"
    _write_phase4_evidence_files(evidence_dir)
