"""Phase5 business destructive cleanup ledger contract."""
...
from scripts.check_phase5_business_destructive_cleanup_gate import LEDGER_HEADER, validate_ledger

REPO_ROOT = Path(__file__).resolve().parents[2]
LEDGER_PATH = REPO_ROOT / "docs" / "architecture" / "phase5-business-destructive-cleanup-ledger.csv"
...
def test_phase5_business_cleanup_ledger_passes_draft_gate() -> None:
    result = validate_ledger(REPO_ROOT, mode="draft")

    assert result.valid, result.details


def test_phase5_business_cleanup_ledger_header_and_entry_set_match_matrix() -> None:
    ...
    matrix_rows = [row for row in _rows(MATRIX_PATH) if row["phase4_carrier"] == "True"]
...
def test_phase5_business_cleanup_ledger_tracks_current_surface_states() -> None:
