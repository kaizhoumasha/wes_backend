...
def test_admission_request_accepts_only_actual_bin_and_scan_time() -> None:
    ...
    assert request.data.bin_code == "BIN-001"


@pytest.mark.parametrize(
    "data",
    [
        {"bin_code": "BIN-001", "scanned_at": 1_788_389_899_900, "expected_bin": "BIN-002"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"result": "NO_WORK"}, "NO_WORK"),
        ({"result": "WAIT", "retry_after_ms": 1000}, "WAIT"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"result": "NO_WORK", "task_id": "PICK-001"},
        {"result": "WAIT", "task_id": "PICK-001", "retry_after_ms": 1000},
    ],
)
def test_admission_response_rejects_mixed_union_fields(data: dict[str, object]) -> None:
    with pytest.raises(ValidationError):
        parse_manual_bin_admission_response(
            200,
            {"operation_id": OPERATION_ID, "code": "DECIDED", "timestamp": 1_788_389_900_010, "data": data},
