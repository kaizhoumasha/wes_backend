...
def _snapshot(
    ...
    *,
    ...
    return {
        ...
        "request_body_digest": request_body_digest or sha256(request_body).hexdigest(),
    }
...
@pytest.mark.asyncio
async def test_oversized_request_is_a_deterministic_request_body_rejection() -> None:
    ...
    assert result.reason_code == "REQUEST_BODY_TOO_LARGE"
...
async def test_submit_wire_uses_the_persisted_operation_snapshot_without_local_caller() -> None:
    ...
    assert client.calls == [
        (
            ...
            _snapshot(operation_id, 1710000000000, payload)["request_body"],
            {
                "max_request_body_bytes": 256 * 1024,
                "max_response_body_bytes": 256 * 1024,
            },
...
@pytest.mark.asyncio
async def test_submit_rejects_a_tampered_persisted_request_body_digest_before_send() -> None:
    transport = NoSendTransport()
    payload = {
        "transport_task_id": "transport-1",
        "kind": "BIN_MOVE",
    ...
    result = await WmsTransportAdapter(WmsClient(transport), submit_path=TRANSPORT_SUBMIT_PATH).submit(
        **_snapshot(
            ...
            1,
            payload,
            request_body_digest="0" * 64,
        )
    )

    assert result.code is TransportSubmitCode.REJECTED
    assert result.reason_code == "REQUEST_BODY_DIGEST_MISMATCH"
    assert transport.send_count == 0
