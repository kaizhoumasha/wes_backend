"""Runtime evidence artifact composer contracts."""
...
COMPOSER_SCRIPT = REPO_ROOT / "scripts" / "compose_runtime_evidence_artifact.py"
...
def test_runtime_evidence_composer_writes_gate_accepted_simulator_artifact(tmp_path) -> None:
    artifact_path = tmp_path / "runtime-evidence-simulator.json"
    ...
    assert "RUNTIME_EVIDENCE_READY" in gate_result.stdout


def test_runtime_evidence_composer_requires_evidence_dir_for_site_profile(tmp_path) -> None:
    artifact_path = tmp_path / "runtime-evidence-site.json"
    ...
    assert "Runtime evidence site/production evidence-dir is required" in compose_result.stdout


def test_runtime_evidence_compose_function_requires_evidence_dir_for_site_profile() -> None:
    import pytest

    from scripts.compose_runtime_evidence_artifact import compose_artifact

    with pytest.raises(ValueError, match="MISSING_RUNTIME_EVIDENCE_DIR"):
        compose_artifact(
            profile="site",
...
def test_runtime_evidence_composer_writes_gate_accepted_site_artifact(tmp_path) -> None:
    artifact_path = tmp_path / "runtime-evidence-site.json"
