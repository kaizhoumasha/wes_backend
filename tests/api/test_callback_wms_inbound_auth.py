...
async def test_fastapi_startup_binds_and_shutdown_clears_the_fixed_wms_policy() -> None:
    ...
    with (
        ...
        async with register.register_init(app):
            ...
            assert not hasattr(app.state, "outbound_picking_runtime")
            for handler_name in (
                "wms_picking_task_issued_handler",
                "wms_picking_task_plan_delta_handler",
                "wms_picking_task_queue_changed_handler",
                "wms_manual_bin_completed_handler",
            ):
                assert getattr(app.state, handler_name) is not None
