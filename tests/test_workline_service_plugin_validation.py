...
from src.workline_plugins.smt_classifier.plugin import SmtClassifierDeviceRole, SmtClassifierPlugin
from src.workline_plugins.smt_classifier.state_machine import SmtClassifierStageMachine
...
def test_workline_model_resolves_runtime_plugin_classes() -> None:
    """WorkLine 应能按 plugin_key 解析运行时插件与状态机。"""
    ...
    assert workline.state_machine_class is SmtClassifierStageMachine


@pytest.mark.asyncio
async def test_workline_service_rejects_plugin_when_required_devices_missing(db_session) -> None:
    """设置插件时，若工作线缺少必需设备角色则拒绝保存。"""
    ...
    with (
        ...
        pytest.raises(BadRequestException, match=r"OUTPUT_ARM.*CONVEYOR"),
    ):
...
@pytest.mark.asyncio
async def test_workline_service_accepts_plugin_when_required_devices_present(db_session) -> None:
    """设备角色满足插件要求时，应允许绑定插件。"""
    ...
    assert result.state_machine_class is SmtClassifierStageMachine
